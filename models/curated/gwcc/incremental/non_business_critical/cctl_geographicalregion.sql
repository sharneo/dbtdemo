...
2026-01-01      0.0                             Incremental Curated model for cctl_geographicalregion.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_geographicalregion')) }}
    FROM {{ ref('stg_cctl_geographicalregion') }}
    {% if is_incremental() %}
