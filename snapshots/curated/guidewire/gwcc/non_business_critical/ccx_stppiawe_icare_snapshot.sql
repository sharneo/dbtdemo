{% snapshot ccx_stppiawe_icare_snapshot %}
...
2026-01-01      0.0                             SCD Type 2 snapshot for ccx_stppiawe_icare.
                                                Source: ref('stg_raw_ccx_stppiawe_icare')
                                                unique_key: stppiawe_icare_sk (surrogate on PK 'id')
...
{{ config(
    target_schema='gwcc',
    unique_key='stppiawe_icare_sk',
    strategy='check',
    alias='ccx_stppiawe_icare',
    check_cols=['hash_key'],
    hard_deletes='new_record',
    tags=['snapshot', 'claim_centre', 'non_business_critical', 'ccx_stppiawe_icare']
) }}
...
FROM {{ ref('stg_raw_ccx_stppiawe_icare') }}
