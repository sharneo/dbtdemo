...
2026-01-01      0.5                             Curated current-state view for bctl_directbillpmntacctrole.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_directbillpmntacctrole_current')) }}
    FROM {{ ref('dt_bctl_directbillpmntacctrole_current') }}
