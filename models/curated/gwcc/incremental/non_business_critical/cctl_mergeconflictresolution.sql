...
2026-01-01      0.0                             Incremental Curated model for cctl_mergeconflictresolution.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_mergeconflictresolution')) }}
    FROM {{ ref('stg_cctl_mergeconflictresolution') }}
    {% if is_incremental() %}
