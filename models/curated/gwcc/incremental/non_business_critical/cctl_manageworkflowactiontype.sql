...
2026-01-01      0.0                             Incremental Curated model for cctl_manageworkflowactiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_manageworkflowactiontype')) }}
    FROM {{ ref('stg_cctl_manageworkflowactiontype') }}
    {% if is_incremental() %}
