...
2026-01-01      0.0                             Incremental Curated model for cctl_reinsuranceflaggedstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_reinsuranceflaggedstatus')) }}
    FROM {{ ref('stg_cctl_reinsuranceflaggedstatus') }}
    {% if is_incremental() %}
