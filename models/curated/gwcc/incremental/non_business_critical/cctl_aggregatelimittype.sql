...
2026-01-01      0.0                             Incremental Curated model for cctl_aggregatelimittype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_aggregatelimittype')) }}
    FROM {{ ref('stg_cctl_aggregatelimittype') }}
    {% if is_incremental() %}
