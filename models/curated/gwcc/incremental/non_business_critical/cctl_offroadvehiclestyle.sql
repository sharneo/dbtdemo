...
2026-01-01      0.0                             Incremental Curated model for cctl_offroadvehiclestyle.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_offroadvehiclestyle')) }}
    FROM {{ ref('stg_cctl_offroadvehiclestyle') }}
    {% if is_incremental() %}
