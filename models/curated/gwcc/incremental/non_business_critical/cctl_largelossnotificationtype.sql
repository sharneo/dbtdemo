...
2026-01-01      0.0                             Incremental Curated model for cctl_largelossnotificationtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_largelossnotificationtype')) }}
    FROM {{ ref('stg_cctl_largelossnotificationtype') }}
    {% if is_incremental() %}
