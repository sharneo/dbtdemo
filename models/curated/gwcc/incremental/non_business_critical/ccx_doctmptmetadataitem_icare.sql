...
2026-01-01      0.0                             Incremental Curated model for ccx_doctmptmetadataitem_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_doctmptmetadataitem_icare')) }}
    FROM {{ ref('stg_ccx_doctmptmetadataitem_icare') }}
    {% if is_incremental() %}
