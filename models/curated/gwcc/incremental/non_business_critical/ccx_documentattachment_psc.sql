...
2026-01-01      0.0                             Incremental Curated model for ccx_documentattachment_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_documentattachment_psc')) }}
    FROM {{ ref('stg_ccx_documentattachment_psc') }}
    {% if is_incremental() %}
