...
2026-01-01      0.0                             Incremental Curated model for cctl_riarrangementtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_riarrangementtype')) }}
    FROM {{ ref('stg_cctl_riarrangementtype') }}
    {% if is_incremental() %}
