...
{#
  ...
  Version History:
  0.1  10/10/18  Q.Nguyen  Initial Draft
  0.2  17/10/18  G.Johnstone  Added PAYMENT_WEEKLYBENEFIT
  0.3  17/10/18  Q.Nguyen  Converted PAYMENT_WEEKLYBENEFIT to dimension
  0.4  13/06/19  V.Lau  Change in table name, Added PAYEE, LAST_POSTING_DT, PAYMENT_PRESENTED_DT
  0.5  03/07/19  V.Lau  Added AUTO_PAYMENT_IND
  0.6  22/02/22  S.Nayeem  Added SOLV_UNIQUE_PAYMENT_ID
  0.7  16/03/22  Yu Yain  TDAREP-1556: Added LATEST_WEEKLY_BENEFIT_IND
  0.8  04-08-2022  Yu Yain  TDAREP-1816: Added MIGRATED_PYMT_IND
  0.9  25-08-2022  Yu Yain  TDAREP-1928: Added DIGITAL_PAYMENT_IND
  0.10  15-09-2022  Yu Yain  TDAREP-1928: Added ocr invoice ID
  0.11  24-04-2023  Yu Yain  TDAREP-2682: Add CAMT Void Date, Westpac ID and Reissue CheckNumber
  0.12  21-05-2023  Yu Yain  TDAREP-2763: Add Void Reason, Voided Pymt #, Transfer To Claim #
#}
...
cctl_gstmethoppel_ext as (
    select
        id,
        typecode,
        name
    from {{ ref('v_cctl_gstmethoppel_ext_current') }}
),
cctl_checktype as (
    select
        id,
...
select
    ...
    case
        when exists (
            ...
            join cc_activitypattern actpattern
...
left join cctl_gstmethoppel_ext gstmth
...
left join ccx_checkreissuanceinfo_icare rei
    inner join cc_check_rei reichq
