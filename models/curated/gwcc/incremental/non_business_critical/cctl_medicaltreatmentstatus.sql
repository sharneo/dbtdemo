...
2026-01-01      0.0                             Incremental Curated model for cctl_medicaltreatmentstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_medicaltreatmentstatus')) }}
    FROM {{ ref('stg_cctl_medicaltreatmentstatus') }}
    {% if is_incremental() %}
