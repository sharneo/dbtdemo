...
2026-01-01      0.0                             Incremental Curated model for cctl_claimsearchnamesearchtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimsearchnamesearchtype')) }}
    FROM {{ ref('stg_cctl_claimsearchnamesearchtype') }}
    {% if is_incremental() %}
