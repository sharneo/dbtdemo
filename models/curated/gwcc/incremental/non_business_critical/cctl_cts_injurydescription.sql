...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_injurydescription.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_injurydescription')) }}
    FROM {{ ref('stg_cctl_cts_injurydescription') }}
    {% if is_incremental() %}
