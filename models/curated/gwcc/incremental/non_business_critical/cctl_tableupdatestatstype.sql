...
2026-01-01      0.0                             Incremental Curated model for cctl_tableupdatestatstype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_tableupdatestatstype')) }}
    FROM {{ ref('stg_cctl_tableupdatestatstype') }}
    {% if is_incremental() %}
