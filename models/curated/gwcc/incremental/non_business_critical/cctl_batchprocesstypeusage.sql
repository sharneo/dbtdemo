...
2026-01-01      0.0                             Incremental Curated model for cctl_batchprocesstypeusage.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_batchprocesstypeusage')) }}
    FROM {{ ref('stg_cctl_batchprocesstypeusage') }}
    {% if is_incremental() %}
