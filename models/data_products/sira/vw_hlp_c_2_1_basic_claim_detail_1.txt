...
with cc_claim as (
    select
        ...
        retired
    from {{ ref('v_cc_claim_current_current') }}
),
...
policy_period as (
    select
        ...
        status,
        periodstart
    from {{ ref('v_pc_policyperiod_current') }}
),
...
training_status_icare as (
    select
        id,
        typecode
    from {{ ref('v_cctl_trainingstatus_icare_current') }}
),
...
claimant as (
    select
        ...
        cntryname,
        gendername
    from {{ ref('v_sira_claimant_current') }}
),
...
dependents as (
    select
        ...
        sum(other) as other
    from {{ ref('v_sira_dependent_current') }}
    group by claimid
),
