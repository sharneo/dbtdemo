...
2026-01-01      0.5                             Curated current-state view for bctl_accountcontext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_accountcontext_current')) }}
    FROM {{ ref('dt_bctl_accountcontext_current') }}
