...
2026-01-01      0.0                             Incremental Curated model for cctl_contactmatchresulttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactmatchresulttype')) }}
    FROM {{ ref('stg_cctl_contactmatchresulttype') }}
    {% if is_incremental() %}
