...
2026-01-01      0.0                             Incremental Curated model for cctl_contactlinkstatustype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactlinkstatustype')) }}
    FROM {{ ref('stg_cctl_contactlinkstatustype') }}
    {% if is_incremental() %}
