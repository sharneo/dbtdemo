...
2026-01-01      0.0                             Incremental Curated model for cctl_basecriterionconfig.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_basecriterionconfig')) }}
    FROM {{ ref('stg_cctl_basecriterionconfig') }}
    {% if is_incremental() %}
