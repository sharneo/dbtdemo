...
2026-01-01      0.0                             Incremental Curated model for cctl_claimmetriccategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimmetriccategory')) }}
    FROM {{ ref('stg_cctl_claimmetriccategory') }}
    {% if is_incremental() %}
