...
2026-01-01      0.0                             Incremental staging model for cc_blobcoldatadist.
                                                blobcoldatadist_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cc_blobcoldatadist"]
) }}
...
(
            ...
            SELECT
                ...
                CAST(data_payload:BlobColName::TEXT AS VARCHAR(30)) AS blobcolname,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cc_blobcoldatadist') }}
            ...
            SELECT
                ...
                CAST($1:blobcolname::TEXT AS VARCHAR(30)) AS blobcolname,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS blobcoldatadist_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'averagelength',
                        'publicid',
                        'minimumlength',
