...
2026-01-01      0.0                             Incremental Curated model for cctl_destructionrequeststatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_destructionrequeststatus')) }}
    FROM {{ ref('stg_cctl_destructionrequeststatus') }}
    {% if is_incremental() %}
