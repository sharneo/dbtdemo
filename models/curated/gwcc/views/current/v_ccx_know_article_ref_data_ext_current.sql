...
2026-01-01      0.5                             Curated current-state view for ccx_know_article_ref_data_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_ccx_know_article_ref_data_ext_current')) }}
    FROM {{ ref('dt_ccx_know_article_ref_data_ext_current') }}
