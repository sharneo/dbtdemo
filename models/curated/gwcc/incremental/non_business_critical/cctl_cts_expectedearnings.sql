...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_expectedearnings.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_expectedearnings')) }}
    FROM {{ ref('stg_cctl_cts_expectedearnings') }}
    {% if is_incremental() %}
