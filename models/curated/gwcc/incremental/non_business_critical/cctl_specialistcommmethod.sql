...
2026-01-01      0.0                             Incremental Curated model for cctl_specialistcommmethod.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_specialistcommmethod')) }}
    FROM {{ ref('stg_cctl_specialistcommmethod') }}
    {% if is_incremental() %}
