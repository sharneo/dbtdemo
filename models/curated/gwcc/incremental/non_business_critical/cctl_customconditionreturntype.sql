...
2026-01-01      0.0                             Incremental Curated model for cctl_customconditionreturntype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_customconditionreturntype')) }}
    FROM {{ ref('stg_cctl_customconditionreturntype') }}
    {% if is_incremental() %}
