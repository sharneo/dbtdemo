...
2026-01-01      0.0                             Incremental Curated model for cctl_mattercourtdistrict.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_mattercourtdistrict')) }}
    FROM {{ ref('stg_cctl_mattercourtdistrict') }}
    {% if is_incremental() %}
