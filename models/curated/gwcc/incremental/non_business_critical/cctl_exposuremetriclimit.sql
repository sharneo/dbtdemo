...
2026-01-01      0.0                             Incremental Curated model for cctl_exposuremetriclimit.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_exposuremetriclimit')) }}
    FROM {{ ref('stg_cctl_exposuremetriclimit') }}
    {% if is_incremental() %}
