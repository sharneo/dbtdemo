...
2026-01-01      0.0                             Incremental staging model for ccx_weekbenefpie_icare.
                                                weekbenefpie_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "business_critical", "ccx_weekbenefpie_icare"]
) }}
...
(
            ...
            SELECT
                ...
                CAST(data_payload:OrdinaryEarnings AS NUMBER(7,2)) AS ordinaryearnings,
                CAST(data_payload:Deductions AS NUMBER(7,2)) AS deductions,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_weekbenefpie_icare') }}
            ...
            SELECT
                ...
                CAST($1:ordinaryearnings AS NUMBER(7,2)) AS ordinaryearnings,
                CAST($1:deductions AS NUMBER(7,2)) AS deductions,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS weekbenefpie_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'createuserid',
                        'exposureid',
