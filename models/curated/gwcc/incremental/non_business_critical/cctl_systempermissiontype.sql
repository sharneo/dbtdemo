...
2026-01-01      0.0                             Incremental Curated model for cctl_systempermissiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_systempermissiontype')) }}
    FROM {{ ref('stg_cctl_systempermissiontype') }}
    {% if is_incremental() %}
