...
2026-01-01      0.0                             Incremental Curated model for cctl_msprecommendation_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_msprecommendation_ext')) }}
    FROM {{ ref('stg_cctl_msprecommendation_ext') }}
    {% if is_incremental() %}
