...
2026-01-01      0.0                             Incremental Curated model for ccx_wpiactpeerreview_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_wpiactpeerreview_ext')) }}
    FROM {{ ref('stg_ccx_wpiactpeerreview_ext') }}
    {% if is_incremental() %}
