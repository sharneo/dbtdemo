...
2026-01-01      0.5                             Curated current-state view for bctl_invoiceitemviewoption.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_invoiceitemviewoption_current')) }}
    FROM {{ ref('dt_bctl_invoiceitemviewoption_current') }}
