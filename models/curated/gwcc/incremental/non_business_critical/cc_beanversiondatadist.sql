...
2026-01-01      0.0                             Incremental Curated model for cc_beanversiondatadist.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_beanversiondatadist')) }}
    FROM {{ ref('stg_cc_beanversiondatadist') }}
    {% if is_incremental() %}
