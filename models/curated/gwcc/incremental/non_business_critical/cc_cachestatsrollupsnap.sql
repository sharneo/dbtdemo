...
2026-01-01      0.0                             Incremental Curated model for cc_cachestatsrollupsnap.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_cachestatsrollupsnap')) }}
    FROM {{ ref('stg_cc_cachestatsrollupsnap') }}
    {% if is_incremental() %}
