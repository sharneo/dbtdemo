...
2026-01-01      0.0                             Incremental Curated model for cctl_wcbenefitfactorcategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wcbenefitfactorcategory')) }}
    FROM {{ ref('stg_cctl_wcbenefitfactorcategory') }}
    {% if is_incremental() %}
