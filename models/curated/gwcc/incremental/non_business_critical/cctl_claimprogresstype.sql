...
2026-01-01      0.0                             Incremental Curated model for cctl_claimprogresstype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimprogresstype')) }}
    FROM {{ ref('stg_cctl_claimprogresstype') }}
    {% if is_incremental() %}
