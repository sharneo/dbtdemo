...
2026-01-01      0.0                             Incremental Curated model for cctl_dynamicactioncategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_dynamicactioncategory')) }}
    FROM {{ ref('stg_cctl_dynamicactioncategory') }}
    {% if is_incremental() %}
