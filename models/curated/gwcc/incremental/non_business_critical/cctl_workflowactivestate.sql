...
2026-01-01      0.0                             Incremental Curated model for cctl_workflowactivestate.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_workflowactivestate')) }}
    FROM {{ ref('stg_cctl_workflowactivestate') }}
    {% if is_incremental() %}
