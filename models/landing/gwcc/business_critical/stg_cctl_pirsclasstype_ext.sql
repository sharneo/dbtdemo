...
2026-06-01      0.0                             Incremental staging model for cctl_pirsclasstype_ext.
                                                pirsclasstype_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["landing", "gwcc", "claim_centre", "business_critical", "cctl_pirsclasstype_ext"]
) }}
...
(
            ...
            FROM {{ source('gwcc', 'cctl_pirsclasstype_ext') }}
...
cte_transformed AS (
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key(['id']) }} AS VARCHAR(150)) AS pirsclasstype_ext_sk,
