...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_wcbenefitpayment.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_wcbenefitpayment')) }}
    FROM {{ ref('stg_cctl_cts_wcbenefitpayment') }}
    {% if is_incremental() %}
