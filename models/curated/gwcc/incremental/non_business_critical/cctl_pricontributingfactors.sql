...
2026-01-01      0.0                             Incremental Curated model for cctl_pricontributingfactors.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_pricontributingfactors')) }}
    FROM {{ ref('stg_cctl_pricontributingfactors') }}
    {% if is_incremental() %}
