...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_nonpecuniarybentype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_nonpecuniarybentype')) }}
    FROM {{ ref('stg_cctl_cts_nonpecuniarybentype') }}
    {% if is_incremental() %}
