...
2026-01-01      0.0                             Incremental Curated model for cctl_claimagent_icare_replica.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimagent_icare_replica')) }}
    FROM {{ ref('stg_cctl_claimagent_icare_replica') }}
    {% if is_incremental() %}
