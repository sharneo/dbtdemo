...
2026-01-01      0.0                             Curated SCD2 full history view for cc_claim.
                                                Source: {{ ref('cc_claim_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "business_critical", "cc_claim"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cc_claim_snapshot')) }}
    FROM {{ ref('cc_claim_snapshot') }}
),
