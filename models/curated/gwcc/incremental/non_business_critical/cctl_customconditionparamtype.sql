...
2026-01-01      0.0                             Incremental Curated model for cctl_customconditionparamtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_customconditionparamtype')) }}
    FROM {{ ref('stg_cctl_customconditionparamtype') }}
    {% if is_incremental() %}
