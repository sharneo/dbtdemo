...
2026-01-01      0.5                             Curated current-state view for cctl_cts_ref_piaweindexation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_cts_ref_piaweindexation_current')) }}
    FROM {{ ref('dt_cctl_cts_ref_piaweindexation_current') }}
