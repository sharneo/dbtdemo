...
2026-01-01      0.0                             Incremental Curated model for cctl_personaldatatagvalue.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_personaldatatagvalue')) }}
    FROM {{ ref('stg_cctl_personaldatatagvalue') }}
    {% if is_incremental() %}
