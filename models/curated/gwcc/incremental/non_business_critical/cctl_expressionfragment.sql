...
2026-01-01      0.0                             Incremental Curated model for cctl_expressionfragment.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_expressionfragment')) }}
    FROM {{ ref('stg_cctl_expressionfragment') }}
    {% if is_incremental() %}
