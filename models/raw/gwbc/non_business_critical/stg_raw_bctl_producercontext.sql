...
2026-01-01      0.0                             Incremental staging model for bctl_producercontext.
                                                producercontext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_billing_centre", "billing_centre", "non_business_critical", "bctl_producercontext"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwbc', 'bctl_producercontext') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS producercontext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        's_en_us_edg',
                        'l_en_us',
                        'priority',
