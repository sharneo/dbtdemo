...
2026-01-01      0.5                             Curated current-state view for bctl_accountstatus_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_accountstatus_ext_current')) }}
    FROM {{ ref('dt_bctl_accountstatus_ext_current') }}
