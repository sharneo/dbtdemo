...
2026-01-01      0.0                             Incremental Curated model for cc_servicerequestdocumentlink.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_servicerequestdocumentlink')) }}
    FROM {{ ref('stg_cc_servicerequestdocumentlink') }}
    {% if is_incremental() %}
