...
2026-01-01      0.0                             Incremental Curated model for cctl_workloadclassification.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_workloadclassification')) }}
    FROM {{ ref('stg_cctl_workloadclassification') }}
    {% if is_incremental() %}
