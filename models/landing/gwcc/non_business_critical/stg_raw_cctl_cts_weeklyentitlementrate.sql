...
2026-01-01      0.0                             Incremental staging model for cctl_cts_weeklyentitlementrate.
                                                cts_weeklyentitlementrate_sk: Entity identity surrogate key on PK ('id')
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cctl_cts_weeklyentitlementrate') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS cts_weeklyentitlementrate_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'l_en_us',
                        'priority',
                        's_en_us_edg_ph',
