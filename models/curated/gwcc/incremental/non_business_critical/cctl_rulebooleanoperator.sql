...
2026-01-01      0.0                             Incremental Curated model for cctl_rulebooleanoperator.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_rulebooleanoperator')) }}
    FROM {{ ref('stg_cctl_rulebooleanoperator') }}
    {% if is_incremental() %}
