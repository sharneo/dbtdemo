...
2026-01-01      0.0                             Incremental Curated model for cctl_assessmentcontentaction.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_assessmentcontentaction')) }}
    FROM {{ ref('stg_cctl_assessmentcontentaction') }}
    {% if is_incremental() %}
