...
2026-01-01      0.5                             Curated current-state view for bctl_paymentrequeststatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_paymentrequeststatus_current')) }}
    FROM {{ ref('dt_bctl_paymentrequeststatus_current') }}
