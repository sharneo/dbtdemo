...
2026-01-01      0.0                             Incremental Curated model for cctl_transactionsearchview.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_transactionsearchview')) }}
    FROM {{ ref('stg_cctl_transactionsearchview') }}
    {% if is_incremental() %}
