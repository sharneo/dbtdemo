...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_concurrentemployment.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_concurrentemployment')) }}
    FROM {{ ref('stg_ccx_cts_concurrentemployment') }}
    {% if is_incremental() %}
