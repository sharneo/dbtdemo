...
2026-01-01      0.0                             Incremental Curated model for ccx_ahrrserviceline_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_ahrrserviceline_icare')) }}
    FROM {{ ref('stg_ccx_ahrrserviceline_icare') }}
    {% if is_incremental() %}
