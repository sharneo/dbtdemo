...
2026-01-01      0.0                             Incremental Curated model for cctl_approvedtreatment.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_approvedtreatment')) }}
    FROM {{ ref('stg_cctl_approvedtreatment') }}
    {% if is_incremental() %}
