...
2026-01-01      0.0                             Curated current-state view for pctl_policyperiodstatus.
                                                Source: {{ ref('pctl_policyperiodstatus_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_current", "policy_centre", "non_business_critical", "pctl_policyperiodstatus"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pctl_policyperiodstatus_snapshot')) }}
    FROM {{ ref('pctl_policyperiodstatus_snapshot') }}
