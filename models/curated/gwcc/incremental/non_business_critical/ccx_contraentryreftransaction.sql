...
2026-01-01      0.0                             Incremental Curated model for ccx_contraentryreftransaction.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_contraentryreftransaction')) }}
    FROM {{ ref('stg_ccx_contraentryreftransaction') }}
    {% if is_incremental() %}
