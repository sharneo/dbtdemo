...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_postsrgrywrkcapdecsn.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_postsrgrywrkcapdecsn')) }}
    FROM {{ ref('stg_cctl_cts_postsrgrywrkcapdecsn') }}
    {% if is_incremental() %}
