...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequesttier.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequesttier')) }}
    FROM {{ ref('stg_cctl_servicerequesttier') }}
    {% if is_incremental() %}
