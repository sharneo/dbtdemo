...
2026-01-01      0.0                             Curated SCD2 full history view for pc_contact.
                                                Source: {{ ref('pc_contact_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "policy_centre", "business_critical", "pc_contact"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pc_contact_snapshot')) }}
    FROM {{ ref('pc_contact_snapshot') }}
),
