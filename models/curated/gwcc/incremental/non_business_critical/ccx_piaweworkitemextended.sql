...
2026-01-01      0.0                             Incremental Curated model for ccx_piaweworkitemextended.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_piaweworkitemextended')) }}
    FROM {{ ref('stg_ccx_piaweworkitemextended') }}
    {% if is_incremental() %}
