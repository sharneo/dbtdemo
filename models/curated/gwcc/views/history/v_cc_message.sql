...
2026-01-01      0.0                             Curated SCD2 full history view for cc_message.
                                                Source: {{ ref('cc_message_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "non_business_critical", "cc_message"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cc_message_snapshot')) }}
    FROM {{ ref('cc_message_snapshot') }}
),
