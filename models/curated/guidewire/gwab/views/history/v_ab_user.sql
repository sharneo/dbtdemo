...
2026-01-01      0.0                             Curated SCD2 full history view for ab_user.
                                                Source: {{ ref('ab_user_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "contact_manager", "non_business_critical", "ab_user"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('ab_user_snapshot')) }}
    FROM {{ ref('ab_user_snapshot') }}
),
