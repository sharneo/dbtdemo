...
2026-01-01      0.0                             Incremental Curated model for cctl_bulkinvoicejobtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_bulkinvoicejobtype')) }}
    FROM {{ ref('stg_cctl_bulkinvoicejobtype') }}
    {% if is_incremental() %}
