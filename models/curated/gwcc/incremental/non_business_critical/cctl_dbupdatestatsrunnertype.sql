...
2026-01-01      0.0                             Incremental Curated model for cctl_dbupdatestatsrunnertype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_dbupdatestatsrunnertype')) }}
    FROM {{ ref('stg_cctl_dbupdatestatsrunnertype') }}
    {% if is_incremental() %}
