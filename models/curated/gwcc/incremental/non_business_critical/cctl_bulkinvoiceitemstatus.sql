...
2026-01-01      0.0                             Incremental Curated model for cctl_bulkinvoiceitemstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_bulkinvoiceitemstatus')) }}
    FROM {{ ref('stg_cctl_bulkinvoiceitemstatus') }}
    {% if is_incremental() %}
