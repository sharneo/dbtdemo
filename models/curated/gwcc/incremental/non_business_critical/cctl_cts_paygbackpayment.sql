...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_paygbackpayment.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_paygbackpayment')) }}
    FROM {{ ref('stg_cctl_cts_paygbackpayment') }}
    {% if is_incremental() %}
