...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_ref_permanentimpts.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_ref_permanentimpts')) }}
    FROM {{ ref('stg_cctl_cts_ref_permanentimpts') }}
    {% if is_incremental() %}
