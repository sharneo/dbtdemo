...
2026-01-01      0.5                             Curated current-state view for pctl_ratetablecolumndisplay.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pctl_ratetablecolumndisplay_current')) }}
    FROM {{ ref('dt_pctl_ratetablecolumndisplay_current') }}
