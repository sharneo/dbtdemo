...
2026-01-01      0.0                             Curated current-state view for pc_policyperiod.
                                                Source: {{ ref('pc_policyperiod_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_current", "policy_centre", "non_business_critical", "pc_policyperiod"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pc_policyperiod_snapshot')) }}
    FROM {{ ref('pc_policyperiod_snapshot') }}
