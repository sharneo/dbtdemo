...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequestquotestatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequestquotestatus')) }}
    FROM {{ ref('stg_cctl_servicerequestquotestatus') }}
    {% if is_incremental() %}
