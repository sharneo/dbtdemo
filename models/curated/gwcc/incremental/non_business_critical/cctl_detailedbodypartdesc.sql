...
2026-01-01      0.0                             Incremental Curated model for cctl_detailedbodypartdesc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_detailedbodypartdesc')) }}
    FROM {{ ref('stg_cctl_detailedbodypartdesc') }}
    {% if is_incremental() %}
