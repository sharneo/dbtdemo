...
2026-01-01      0.0                             Incremental Curated model for cctl_claimindicatorcriterion.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimindicatorcriterion')) }}
    FROM {{ ref('stg_cctl_claimindicatorcriterion') }}
    {% if is_incremental() %}
