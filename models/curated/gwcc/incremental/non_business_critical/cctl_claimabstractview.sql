...
2026-01-01      0.0                             Incremental Curated model for cctl_claimabstractview.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimabstractview')) }}
    FROM {{ ref('stg_cctl_claimabstractview') }}
    {% if is_incremental() %}
