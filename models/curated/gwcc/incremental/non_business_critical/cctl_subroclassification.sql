...
2026-01-01      0.0                             Incremental Curated model for cctl_subroclassification.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_subroclassification')) }}
    FROM {{ ref('stg_cctl_subroclassification') }}
    {% if is_incremental() %}
