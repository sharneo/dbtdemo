...
2026-01-01      0.0                             Incremental Curated model for cctl_contactapprovalstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactapprovalstatus')) }}
    FROM {{ ref('stg_cctl_contactapprovalstatus') }}
    {% if is_incremental() %}
