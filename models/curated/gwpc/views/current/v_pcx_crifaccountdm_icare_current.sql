...
2026-01-01      0.5                             Curated current-state view for pcx_crifaccountdm_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pcx_crifaccountdm_icare_current')) }}
    FROM {{ ref('dt_pcx_crifaccountdm_icare_current') }}
