...
2026-01-01      0.0                             Incremental staging model for ccx_camt053mispresentchk_ext.
                                                camt053mispresentchk_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_camt053mispresentchk_ext"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS camt053mispresentchk_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
