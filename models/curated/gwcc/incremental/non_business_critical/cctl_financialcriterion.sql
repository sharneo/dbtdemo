...
2026-01-01      0.0                             Incremental Curated model for cctl_financialcriterion.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_financialcriterion')) }}
    FROM {{ ref('stg_cctl_financialcriterion') }}
    {% if is_incremental() %}
