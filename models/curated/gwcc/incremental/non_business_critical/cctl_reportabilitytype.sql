...
2026-01-01      0.0                             Incremental Curated model for cctl_reportabilitytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_reportabilitytype')) }}
    FROM {{ ref('stg_cctl_reportabilitytype') }}
    {% if is_incremental() %}
