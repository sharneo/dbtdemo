...
2026-01-01      0.0                             Incremental Curated model for cctl_disabledduetoaccident.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_disabledduetoaccident')) }}
    FROM {{ ref('stg_cctl_disabledduetoaccident') }}
    {% if is_incremental() %}
