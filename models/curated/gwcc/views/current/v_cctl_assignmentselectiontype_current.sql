...
2026-01-01      0.5                             Curated current-state view for cctl_assignmentselectiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_assignmentselectiontype_current')) }}
    FROM {{ ref('dt_cctl_assignmentselectiontype_current') }}
