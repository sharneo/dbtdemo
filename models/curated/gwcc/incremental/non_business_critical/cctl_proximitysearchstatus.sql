...
2026-01-01      0.0                             Incremental Curated model for cctl_proximitysearchstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_proximitysearchstatus')) }}
    FROM {{ ref('stg_cctl_proximitysearchstatus') }}
    {% if is_incremental() %}
