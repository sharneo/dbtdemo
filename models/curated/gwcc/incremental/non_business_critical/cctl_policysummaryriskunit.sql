...
2026-01-01      0.0                             Incremental Curated model for cctl_policysummaryriskunit.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_policysummaryriskunit')) }}
    FROM {{ ref('stg_cctl_policysummaryriskunit') }}
    {% if is_incremental() %}
