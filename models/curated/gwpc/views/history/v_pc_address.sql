...
2026-01-01      0.0                             Curated SCD2 full history view for pc_address.
                                                Source: {{ ref('pc_address_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "policy_centre", "business_critical", "pc_address"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pc_address_snapshot')) }}
    FROM {{ ref('pc_address_snapshot') }}
),
