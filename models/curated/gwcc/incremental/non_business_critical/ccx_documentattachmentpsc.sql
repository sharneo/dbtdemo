...
2026-01-01      0.0                             Incremental Curated model for ccx_documentattachmentpsc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_documentattachmentpsc')) }}
    FROM {{ ref('stg_ccx_documentattachmentpsc') }}
    {% if is_incremental() %}
