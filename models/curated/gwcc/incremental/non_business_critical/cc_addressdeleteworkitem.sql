...
2026-01-01      0.0                             Incremental Curated model for cc_addressdeleteworkitem.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_addressdeleteworkitem')) }}
    FROM {{ ref('stg_cc_addressdeleteworkitem') }}
    {% if is_incremental() %}
