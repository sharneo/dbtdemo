...
2026-01-01      0.0                             Incremental Curated model for cc_servicerequestmetriclimit.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_servicerequestmetriclimit')) }}
    FROM {{ ref('stg_cc_servicerequestmetriclimit') }}
    {% if is_incremental() %}
