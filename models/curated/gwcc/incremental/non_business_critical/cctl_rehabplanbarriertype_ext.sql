...
2026-01-01      0.0                             Incremental Curated model for cctl_rehabplanbarriertype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_rehabplanbarriertype_ext')) }}
    FROM {{ ref('stg_cctl_rehabplanbarriertype_ext') }}
    {% if is_incremental() %}
