...
2026-01-01      0.0                             Incremental Curated model for cc_assignableforkeydatadist.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_assignableforkeydatadist')) }}
    FROM {{ ref('stg_cc_assignableforkeydatadist') }}
    {% if is_incremental() %}
