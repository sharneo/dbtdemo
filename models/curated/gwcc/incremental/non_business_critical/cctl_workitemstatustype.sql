...
2026-01-01      0.0                             Incremental Curated model for cctl_workitemstatustype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_workitemstatustype')) }}
    FROM {{ ref('stg_cctl_workitemstatustype') }}
    {% if is_incremental() %}
