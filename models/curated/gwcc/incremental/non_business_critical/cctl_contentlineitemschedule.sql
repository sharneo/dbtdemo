...
2026-01-01      0.0                             Incremental Curated model for cctl_contentlineitemschedule.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contentlineitemschedule')) }}
    FROM {{ ref('stg_cctl_contentlineitemschedule') }}
    {% if is_incremental() %}
