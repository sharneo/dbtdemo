...
2026-01-01      0.5                             Curated current-state view for cctl_servicereqmetriclimittype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_servicereqmetriclimittype_current')) }}
    FROM {{ ref('dt_cctl_servicereqmetriclimittype_current') }}
