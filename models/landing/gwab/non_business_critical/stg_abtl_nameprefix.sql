...
2026-01-01      0.0                             Incremental staging model for abtl_nameprefix.
                                                nameprefix_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["layer:stg", "platform:guidewire","module:gwab","domain:contact_manager", "criticality:non_business_critical", "entity:abtl_nameprefix"]
) }}
...
(
            ...
            FROM {{ source('gwab', 'abtl_nameprefix') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS nameprefix_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'l_en_us',
                        'priority',
                        's_en_us_edg_ph',
