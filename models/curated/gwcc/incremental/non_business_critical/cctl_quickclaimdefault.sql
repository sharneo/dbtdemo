...
2026-01-01      0.0                             Incremental Curated model for cctl_quickclaimdefault.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_quickclaimdefault')) }}
    FROM {{ ref('stg_cctl_quickclaimdefault') }}
    {% if is_incremental() %}
