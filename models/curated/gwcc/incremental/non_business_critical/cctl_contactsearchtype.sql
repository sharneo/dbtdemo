...
2026-01-01      0.0                             Incremental Curated model for cctl_contactsearchtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactsearchtype')) }}
    FROM {{ ref('stg_cctl_contactsearchtype') }}
    {% if is_incremental() %}
