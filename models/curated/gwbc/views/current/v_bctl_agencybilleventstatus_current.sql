...
2026-01-01      0.5                             Curated current-state view for bctl_agencybilleventstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_agencybilleventstatus_current')) }}
    FROM {{ ref('dt_bctl_agencybilleventstatus_current') }}
