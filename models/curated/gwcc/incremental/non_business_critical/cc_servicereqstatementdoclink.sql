...
2026-01-01      0.0                             Incremental Curated model for cc_servicereqstatementdoclink.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_servicereqstatementdoclink')) }}
    FROM {{ ref('stg_cc_servicereqstatementdoclink') }}
    {% if is_incremental() %}
