...
2026-01-01      0.0                             Incremental Curated model for cctl_futuremedicalactiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_futuremedicalactiontype')) }}
    FROM {{ ref('stg_cctl_futuremedicalactiontype') }}
    {% if is_incremental() %}
