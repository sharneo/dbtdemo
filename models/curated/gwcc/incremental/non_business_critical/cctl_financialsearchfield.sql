...
2026-01-01      0.0                             Incremental Curated model for cctl_financialsearchfield.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_financialsearchfield')) }}
    FROM {{ ref('stg_cctl_financialsearchfield') }}
    {% if is_incremental() %}
