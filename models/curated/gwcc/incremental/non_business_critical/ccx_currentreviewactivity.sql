...
2026-01-01      0.0                             Incremental Curated model for ccx_currentreviewactivity.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_currentreviewactivity')) }}
    FROM {{ ref('stg_ccx_currentreviewactivity') }}
    {% if is_incremental() %}
