...
2026-01-01      0.0                             Incremental Curated model for cctl_rehabitemowner_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_rehabitemowner_ext')) }}
    FROM {{ ref('stg_cctl_rehabitemowner_ext') }}
    {% if is_incremental() %}
