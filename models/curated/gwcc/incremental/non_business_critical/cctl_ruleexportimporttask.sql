...
2026-01-01      0.0                             Incremental Curated model for cctl_ruleexportimporttask.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_ruleexportimporttask')) }}
    FROM {{ ref('stg_cctl_ruleexportimporttask') }}
    {% if is_incremental() %}
