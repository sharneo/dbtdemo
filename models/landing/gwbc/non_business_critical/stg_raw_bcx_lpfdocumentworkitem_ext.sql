...
2026-01-01      0.0                             Incremental staging model for bcx_lpfdocumentworkitem_ext.
                                                lpfdocumentworkitem_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_billing_centre", "billing_centre", "non_business_critical", "bcx_lpfdocumentworkitem_ext"]
) }}
...
(
            ...
            SELECT
                ...
                TO_TIMESTAMP_TZ(data_payload:RunDate::NUMBER/1000) AS rundate,
                ...
                'AVRO' file_type
            FROM {{ source('gwbc', 'bcx_lpfdocumentworkitem_ext') }}
            ...
            SELECT
                ...
                $1:rundate::TIMESTAMP_TZ AS rundate,
                $1:id::NUMBER AS id,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS lpfdocumentworkitem_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'processhistoryid',
                        'publicid',
