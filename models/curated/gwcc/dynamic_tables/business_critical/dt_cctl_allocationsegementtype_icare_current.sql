...
2026-01-01      0.5                             Dynamic Table for current-state of cctl_allocationsegementtype_icare.
...
{{ config(
    ...
    tags=["curated", "gwcc", "claim_centre", "business_critical", "dynamic_table"]
) }}

SELECT
    {{ dbt_utils.star(from=ref('v_cctl_allocationsegementtype_icare')) }}
FROM {{ ref('v_cctl_allocationsegementtype_icare') }}
