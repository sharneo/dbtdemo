...
2026-01-01      0.0                             Incremental Curated model for cctl_seccontributingfactors.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_seccontributingfactors')) }}
    FROM {{ ref('stg_cctl_seccontributingfactors') }}
    {% if is_incremental() %}
