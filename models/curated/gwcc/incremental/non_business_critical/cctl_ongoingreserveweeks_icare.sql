...
2026-01-01      0.0                             Incremental Curated model for cctl_ongoingreserveweeks_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_ongoingreserveweeks_icare')) }}
    FROM {{ ref('stg_cctl_ongoingreserveweeks_icare') }}
    {% if is_incremental() %}
