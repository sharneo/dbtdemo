...
2026-01-01      0.0                             Incremental Curated model for ccx_prtlclaimupd_icare__note.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_prtlclaimupd_icare__note')) }}
    FROM {{ ref('stg_ccx_prtlclaimupd_icare__note') }}
    {% if is_incremental() %}
