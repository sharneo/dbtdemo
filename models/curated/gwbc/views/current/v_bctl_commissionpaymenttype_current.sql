...
2026-01-01      0.5                             Curated current-state view for bctl_commissionpaymenttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_commissionpaymenttype_current')) }}
    FROM {{ ref('dt_bctl_commissionpaymenttype_current') }}
