...
2026-01-01      0.0                             Incremental staging model for ccx_wcdassessdetails_icare.
                                                wcdassessdetails_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_wcdassessdetails_icare"]
) }}
...
(
            ...
            SELECT
                ...
                data_payload:ProbableOutcome::NUMBER AS probableoutcome,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_wcdassessdetails_icare') }}
            ...
            SELECT
                ...
                $1:probableoutcome::NUMBER AS probableoutcome,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS wcdassessdetails_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
