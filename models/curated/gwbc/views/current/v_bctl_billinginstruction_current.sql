...
2026-01-01      0.5                             Curated current-state view for bctl_billinginstruction.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_billinginstruction_current')) }}
    FROM {{ ref('dt_bctl_billinginstruction_current') }}
