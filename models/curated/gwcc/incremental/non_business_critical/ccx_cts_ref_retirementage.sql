...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_ref_retirementage.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_ref_retirementage')) }}
    FROM {{ ref('stg_ccx_cts_ref_retirementage') }}
    {% if is_incremental() %}
