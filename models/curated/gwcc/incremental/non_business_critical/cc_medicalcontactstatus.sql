...
2026-01-01      0.0                             Incremental Curated model for cc_medicalcontactstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_medicalcontactstatus')) }}
    FROM {{ ref('stg_cc_medicalcontactstatus') }}
    {% if is_incremental() %}
