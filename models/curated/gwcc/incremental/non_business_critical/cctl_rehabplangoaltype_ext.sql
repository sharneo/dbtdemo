...
2026-01-01      0.0                             Incremental Curated model for cctl_rehabplangoaltype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_rehabplangoaltype_ext')) }}
    FROM {{ ref('stg_cctl_rehabplangoaltype_ext') }}
    {% if is_incremental() %}
