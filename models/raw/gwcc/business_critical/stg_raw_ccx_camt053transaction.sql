...
2026-01-01      0.0                             Incremental staging model for ccx_camt053transaction.
                                                camt053transaction_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "business_critical", "ccx_camt053transaction"]
) }}
...
(
            ...
            SELECT
                ...
                data_payload:MatchedPayment::NUMBER AS matchedpayment,
                CAST(data_payload:MerchantID::TEXT AS VARCHAR(255)) AS merchantid,
                data_payload:ResolutionType::NUMBER AS resolutiontype,
                ...
                data_payload:ReasonToUnknown::NUMBER AS reasontounknown,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_camt053transaction') }}
            ...
            SELECT
                ...
                $1:matchedpayment::NUMBER AS matchedpayment,
                CAST($1:merchantid::TEXT AS VARCHAR(255)) AS merchantid,
                $1:resolutiontype::NUMBER AS resolutiontype,
                ...
                $1:reasontounknown::NUMBER AS reasontounknown,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS camt053transaction_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'reversalindicator',
