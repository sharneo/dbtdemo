...
2026-01-01      0.5                             Curated current-state view for cctl_activitytype_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_activitytype_icare_current')) }}
    FROM {{ ref('dt_cctl_activitytype_icare_current') }}
