...
2026-01-01      0.0                             Curated SCD2 full history view for cctl_state.
                                                Source: {{ ref('cctl_state_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "business_critical", "cctl_state"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cctl_state_snapshot')) }}
    FROM {{ ref('cctl_state_snapshot') }}
),
