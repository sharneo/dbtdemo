...
2026-01-01      0.0                             Incremental Curated model for cctl_propertylineitemcategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_propertylineitemcategory')) }}
    FROM {{ ref('stg_cctl_propertylineitemcategory') }}
    {% if is_incremental() %}
