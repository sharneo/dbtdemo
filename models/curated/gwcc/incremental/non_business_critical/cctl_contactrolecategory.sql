...
2026-01-01      0.0                             Incremental Curated model for cctl_contactrolecategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactrolecategory')) }}
    FROM {{ ref('stg_cctl_contactrolecategory') }}
    {% if is_incremental() %}
