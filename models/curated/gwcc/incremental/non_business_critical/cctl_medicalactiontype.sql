...
2026-01-01      0.0                             Incremental Curated model for cctl_medicalactiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_medicalactiontype')) }}
    FROM {{ ref('stg_cctl_medicalactiontype') }}
    {% if is_incremental() %}
