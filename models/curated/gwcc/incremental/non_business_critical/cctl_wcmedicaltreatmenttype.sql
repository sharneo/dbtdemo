...
2026-01-01      0.0                             Incremental Curated model for cctl_wcmedicaltreatmenttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wcmedicaltreatmenttype')) }}
    FROM {{ ref('stg_cctl_wcmedicaltreatmenttype') }}
    {% if is_incremental() %}
