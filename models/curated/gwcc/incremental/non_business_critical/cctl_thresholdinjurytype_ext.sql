...
2026-01-01      0.0                             Incremental Curated model for cctl_thresholdinjurytype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_thresholdinjurytype_ext')) }}
    FROM {{ ref('stg_cctl_thresholdinjurytype_ext') }}
    {% if is_incremental() %}
