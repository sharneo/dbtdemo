...
2026-01-01      0.5                             Curated current-state view for cctl_assessmentcontentaction.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_assessmentcontentaction_current')) }}
    FROM {{ ref('dt_cctl_assessmentcontentaction_current') }}
