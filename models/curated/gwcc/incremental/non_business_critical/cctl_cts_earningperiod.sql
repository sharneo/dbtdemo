...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_earningperiod.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_earningperiod')) }}
    FROM {{ ref('stg_cctl_cts_earningperiod') }}
    {% if is_incremental() %}
