...
2026-01-01      0.0                             Incremental Curated model for cctl_servicereqmetriclimittype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicereqmetriclimittype')) }}
    FROM {{ ref('stg_cctl_servicereqmetriclimittype') }}
    {% if is_incremental() %}
