...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_ref_paygtaxdata.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_ref_paygtaxdata')) }}
    FROM {{ ref('stg_ccx_cts_ref_paygtaxdata') }}
    {% if is_incremental() %}
