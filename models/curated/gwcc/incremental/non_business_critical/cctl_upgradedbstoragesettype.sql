...
2026-01-01      0.0                             Incremental Curated model for cctl_upgradedbstoragesettype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_upgradedbstoragesettype')) }}
    FROM {{ ref('stg_cctl_upgradedbstoragesettype') }}
    {% if is_incremental() %}
