...
2026-01-01      0.0                             Incremental Curated model for cctl_messagedestinationstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_messagedestinationstatus')) }}
    FROM {{ ref('stg_cctl_messagedestinationstatus') }}
    {% if is_incremental() %}
