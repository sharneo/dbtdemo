...
2026-01-01      0.5                             Curated current-state view for pcx_sportinjurywic_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pcx_sportinjurywic_icare_current')) }}
    FROM {{ ref('dt_pcx_sportinjurywic_icare_current') }}
