...
2026-01-01      0.0                             Incremental staging model for cc_contactcontact.
                                                contactcontact_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cc_contactcontact"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS contactcontact_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'externallinkid',
                        'loadcommandid',
                        'sourcecontactid',
