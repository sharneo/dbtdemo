...
2026-01-01      0.0                             Incremental Curated model for cctl_vehiclepolicystatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_vehiclepolicystatus')) }}
    FROM {{ ref('stg_cctl_vehiclepolicystatus') }}
    {% if is_incremental() %}
