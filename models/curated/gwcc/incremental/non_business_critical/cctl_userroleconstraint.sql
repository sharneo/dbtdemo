...
2026-01-01      0.0                             Incremental Curated model for cctl_userroleconstraint.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_userroleconstraint')) }}
    FROM {{ ref('stg_cctl_userroleconstraint') }}
    {% if is_incremental() %}
