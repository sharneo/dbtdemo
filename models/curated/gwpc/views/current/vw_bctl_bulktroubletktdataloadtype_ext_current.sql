...
2026.02.26      2.0                             Creates Current View for Curated Layer  
                                            bctl_bulktroubletktdataloadtype_ext - Latest record per ID  
...
{{ config(
    materialized='view',
    tags=["daily", "curated", "curated_view", "hourly", "billing"]
) }}

{% set relation = ref('bctl_bulktroubletktdataloadtype_ext') %}
