...
2026-01-01      0.0                             Incremental Curated model for cctl_criterionoperator.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_criterionoperator')) }}
    FROM {{ ref('stg_cctl_criterionoperator') }}
    {% if is_incremental() %}
