...
2026-01-01      0.0                             Incremental Curated model for cc_assignableforkeysizecntdd.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_assignableforkeysizecntdd')) }}
    FROM {{ ref('stg_cc_assignableforkeysizecntdd') }}
    {% if is_incremental() %}
