...
2026-01-01      0.0                             Incremental Curated model for cc_allocatedclaimnumber.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_allocatedclaimnumber')) }}
    FROM {{ ref('stg_cc_allocatedclaimnumber') }}
    {% if is_incremental() %}
