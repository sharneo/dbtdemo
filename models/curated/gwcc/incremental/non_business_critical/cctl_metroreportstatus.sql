...
2026-01-01      0.0                             Incremental Curated model for cctl_metroreportstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_metroreportstatus')) }}
    FROM {{ ref('stg_cctl_metroreportstatus') }}
    {% if is_incremental() %}
