...
2026-01-01      0.0                             Incremental Curated model for ccx_know_article_ref_data_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_know_article_ref_data_ext')) }}
    FROM {{ ref('stg_ccx_know_article_ref_data_ext') }}
    {% if is_incremental() %}
