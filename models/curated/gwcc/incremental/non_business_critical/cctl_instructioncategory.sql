...
2026-01-01      0.0                             Incremental Curated model for cctl_instructioncategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_instructioncategory')) }}
    FROM {{ ref('stg_cctl_instructioncategory') }}
    {% if is_incremental() %}
