...
2026-01-01      0.5                             Curated current-state view for ccx_documentattachment_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_ccx_documentattachment_psc_current')) }}
    FROM {{ ref('dt_ccx_documentattachment_psc_current') }}
