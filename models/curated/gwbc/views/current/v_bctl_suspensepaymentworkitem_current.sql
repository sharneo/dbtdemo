...
2026-01-01      0.5                             Curated current-state view for bctl_suspensepaymentworkitem.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_suspensepaymentworkitem_current')) }}
    FROM {{ ref('dt_bctl_suspensepaymentworkitem_current') }}
