...
2026-01-01      0.0                             Incremental Curated model for cc_servicereqstatementline.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_servicereqstatementline')) }}
    FROM {{ ref('stg_cc_servicereqstatementline') }}
    {% if is_incremental() %}
