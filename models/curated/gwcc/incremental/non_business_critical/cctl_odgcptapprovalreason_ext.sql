...
2026-01-01      0.0                             Incremental Curated model for cctl_odgcptapprovalreason_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_odgcptapprovalreason_ext')) }}
    FROM {{ ref('stg_cctl_odgcptapprovalreason_ext') }}
    {% if is_incremental() %}
