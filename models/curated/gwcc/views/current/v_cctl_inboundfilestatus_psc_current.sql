...
2026-01-01      0.5                             Curated current-state view for cctl_inboundfilestatus_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_inboundfilestatus_psc_current')) }}
    FROM {{ ref('dt_cctl_inboundfilestatus_psc_current') }}
