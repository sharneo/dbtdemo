...
2026-01-01      0.0                             Incremental Curated model for cctl_vehiclelineitemcategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_vehiclelineitemcategory')) }}
    FROM {{ ref('stg_cctl_vehiclelineitemcategory') }}
    {% if is_incremental() %}
