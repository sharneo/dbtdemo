...
2026-01-01      0.0                             Incremental Curated model for cctl_authoritylimittype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_authoritylimittype')) }}
    FROM {{ ref('stg_cctl_authoritylimittype') }}
    {% if is_incremental() %}
