...
2026-01-01      0.0                             Incremental Curated model for cc_policytypemetriclimits.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_policytypemetriclimits')) }}
    FROM {{ ref('stg_cc_policytypemetriclimits') }}
    {% if is_incremental() %}
