...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequestmetric.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequestmetric')) }}
    FROM {{ ref('stg_cctl_servicerequestmetric') }}
    {% if is_incremental() %}
