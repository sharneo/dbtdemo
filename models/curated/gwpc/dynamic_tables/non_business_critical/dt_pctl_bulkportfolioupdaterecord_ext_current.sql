...
2026-01-01      0.5                             Dynamic Table for current-state of pctl_bulkportfolioupdaterecord_ext.
...
{{ config(
    ...
    tags=["curated", "gwpc", "policy_centre", "non_business_critical", "dynamic_table"]
) }}

SELECT
    {{ dbt_utils.star(from=ref('v_pctl_bulkportfolioupdaterecord_ext')) }}
FROM {{ ref('v_pctl_bulkportfolioupdaterecord_ext') }}
