...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequestkind.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequestkind')) }}
    FROM {{ ref('stg_cctl_servicerequestkind') }}
    {% if is_incremental() %}
