...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_entitlementratetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_entitlementratetype')) }}
    FROM {{ ref('stg_cctl_cts_entitlementratetype') }}
    {% if is_incremental() %}
