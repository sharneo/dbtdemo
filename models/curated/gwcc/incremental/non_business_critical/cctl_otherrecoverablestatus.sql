...
2026-01-01      0.0                             Incremental Curated model for cctl_otherrecoverablestatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_otherrecoverablestatus')) }}
    FROM {{ ref('stg_cctl_otherrecoverablestatus') }}
    {% if is_incremental() %}
