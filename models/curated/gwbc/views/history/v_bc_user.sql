...
2026-01-01      0.0                             Curated SCD2 full history view for bc_user.
                                                Source: {{ ref('bc_user_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "billing_centre", "non_business_critical", "bc_user"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('bc_user_snapshot')) }}
    FROM {{ ref('bc_user_snapshot') }}
),
