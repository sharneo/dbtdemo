...
2026-01-01      0.0                             Incremental staging model for pc_paymentplansummary.
                                                paymentplansummary_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "non_business_critical", "pc_paymentplansummary"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS paymentplansummary_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'notes',
                        'publicid',
                        'total',
