...
2026-01-01      0.0                             Incremental Curated model for cctl_riagreementcoveragetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_riagreementcoveragetype')) }}
    FROM {{ ref('stg_cctl_riagreementcoveragetype') }}
    {% if is_incremental() %}
