...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_wcbenefitlineitem.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_wcbenefitlineitem')) }}
    FROM {{ ref('stg_cctl_cts_wcbenefitlineitem') }}
    {% if is_incremental() %}
