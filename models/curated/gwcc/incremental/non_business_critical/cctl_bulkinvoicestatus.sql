...
2026-01-01      0.0                             Incremental Curated model for cctl_bulkinvoicestatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_bulkinvoicestatus')) }}
    FROM {{ ref('stg_cctl_bulkinvoicestatus') }}
    {% if is_incremental() %}
