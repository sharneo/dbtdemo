...
2026-01-01      0.0                             Incremental Curated model for ccx_assessedpirsscore_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_assessedpirsscore_ext')) }}
    FROM {{ ref('stg_ccx_assessedpirsscore_ext') }}
    {% if is_incremental() %}
