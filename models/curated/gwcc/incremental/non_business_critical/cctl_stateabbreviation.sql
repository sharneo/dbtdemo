...
2026-01-01      0.0                             Incremental Curated model for cctl_stateabbreviation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_stateabbreviation')) }}
    FROM {{ ref('stg_cctl_stateabbreviation') }}
    {% if is_incremental() %}
