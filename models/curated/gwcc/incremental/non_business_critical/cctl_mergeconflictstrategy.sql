...
2026-01-01      0.0                             Incremental Curated model for cctl_mergeconflictstrategy.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_mergeconflictstrategy')) }}
    FROM {{ ref('stg_cctl_mergeconflictstrategy') }}
    {% if is_incremental() %}
