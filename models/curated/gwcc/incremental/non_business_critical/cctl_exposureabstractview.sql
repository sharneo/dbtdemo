...
2026-01-01      0.0                             Incremental Curated model for cctl_exposureabstractview.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_exposureabstractview')) }}
    FROM {{ ref('stg_cctl_exposureabstractview') }}
    {% if is_incremental() %}
