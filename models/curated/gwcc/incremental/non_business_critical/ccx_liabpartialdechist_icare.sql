...
2026-01-01      0.0                             Incremental Curated model for ccx_liabpartialdechist_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_liabpartialdechist_icare')) }}
    FROM {{ ref('stg_ccx_liabpartialdechist_icare') }}
    {% if is_incremental() %}
