...
2026-01-01      0.0                             Incremental Curated model for cctl_wcbenefitfactortype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wcbenefitfactortype')) }}
    FROM {{ ref('stg_cctl_wcbenefitfactortype') }}
    {% if is_incremental() %}
