...
2026-01-01      0.0                             Incremental Curated model for cctl_financialthreshold.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_financialthreshold')) }}
    FROM {{ ref('stg_cctl_financialthreshold') }}
    {% if is_incremental() %}
