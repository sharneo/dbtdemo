...
2026-01-01      0.0                             Incremental Curated model for cctl_inboundfiletype_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_inboundfiletype_psc')) }}
    FROM {{ ref('stg_cctl_inboundfiletype_psc') }}
    {% if is_incremental() %}
