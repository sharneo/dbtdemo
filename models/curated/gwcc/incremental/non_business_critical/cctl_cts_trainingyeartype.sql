...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_trainingyeartype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_trainingyeartype')) }}
    FROM {{ ref('stg_cctl_cts_trainingyeartype') }}
    {% if is_incremental() %}
