...
2026-01-01      0.0                             Incremental staging model for pcx_directwage_icare.
                                                directwage_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "business_critical", "pcx_directwage_icare"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS directwage_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'legacyddl',
                        'auditedappwages',
