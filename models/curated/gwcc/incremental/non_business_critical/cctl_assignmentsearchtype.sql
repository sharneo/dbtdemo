...
2026-01-01      0.0                             Incremental Curated model for cctl_assignmentsearchtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_assignmentsearchtype')) }}
    FROM {{ ref('stg_cctl_assignmentsearchtype') }}
    {% if is_incremental() %}
