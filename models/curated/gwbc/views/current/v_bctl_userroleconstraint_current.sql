...
2026-01-01      0.5                             Curated current-state view for bctl_userroleconstraint.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_userroleconstraint_current')) }}
    FROM {{ ref('dt_bctl_userroleconstraint_current') }}
