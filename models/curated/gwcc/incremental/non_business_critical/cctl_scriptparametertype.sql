...
2026-01-01      0.0                             Incremental Curated model for cctl_scriptparametertype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_scriptparametertype')) }}
    FROM {{ ref('stg_cctl_scriptparametertype') }}
    {% if is_incremental() %}
