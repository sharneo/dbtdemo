...
2026-01-01      0.0                             Incremental Curated model for cctl_userexperiencetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_userexperiencetype')) }}
    FROM {{ ref('stg_cctl_userexperiencetype') }}
    {% if is_incremental() %}
