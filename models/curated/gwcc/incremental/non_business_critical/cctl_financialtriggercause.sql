...
2026-01-01      0.0                             Incremental Curated model for cctl_financialtriggercause.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_financialtriggercause')) }}
    FROM {{ ref('stg_cctl_financialtriggercause') }}
    {% if is_incremental() %}
