...
2026-01-01      0.0                             Incremental staging model for pcx_inboundrecordworkitem_psc.
                                                inboundrecordworkitem_psc_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "non_business_critical", "pcx_inboundrecordworkitem_psc"]
) }}
...
(
            ...
            SELECT
                ...
                data_payload:InboundRecord::NUMBER AS inboundrecord,
                ...
                'AVRO' file_type
            FROM {{ source('gwpc', 'pcx_inboundrecordworkitem_psc') }}
            ...
            SELECT
                ...
                $1:inboundrecord::NUMBER AS inboundrecord,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS inboundrecordworkitem_psc_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'processhistoryid',
                        'publicid',
