...
2026-01-01      0.0                             Incremental staging model for pc_policyuserroleassign.
                                                policyuserroleassign_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "business_critical", "pc_policyuserroleassign"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS policyuserroleassign_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'createuserid',
                        'previousgroupid',
                        'publicid',
