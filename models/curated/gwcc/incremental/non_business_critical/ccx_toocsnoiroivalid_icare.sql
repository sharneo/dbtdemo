...
2026-01-01      0.0                             Incremental Curated model for ccx_toocsnoiroivalid_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_toocsnoiroivalid_icare')) }}
    FROM {{ ref('stg_ccx_toocsnoiroivalid_icare') }}
    {% if is_incremental() %}
