...
2026-01-01      0.0                             Incremental Curated model for cctl_assignmentselectiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_assignmentselectiontype')) }}
    FROM {{ ref('stg_cctl_assignmentselectiontype') }}
    {% if is_incremental() %}
