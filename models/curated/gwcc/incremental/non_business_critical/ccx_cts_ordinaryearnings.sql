...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_ordinaryearnings.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_ordinaryearnings')) }}
    FROM {{ ref('stg_ccx_cts_ordinaryearnings') }}
    {% if is_incremental() %}
