...
2026-01-01      0.0                             Incremental Curated model for cctl_wcdetailedinjurytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wcdetailedinjurytype')) }}
    FROM {{ ref('stg_cctl_wcdetailedinjurytype') }}
    {% if is_incremental() %}
