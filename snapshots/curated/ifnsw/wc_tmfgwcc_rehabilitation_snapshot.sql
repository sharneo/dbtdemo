{% snapshot wc_tmfgwcc_rehabilitation_snapshot %}
...
2026.01.11      0.0                             This Creates a  View for the Snapshot for the Table wc_tmfgwcc_rehabilitation_snapshot . 
...
{{ config(
    ...
    unique_key='wc_tmfgwcc_rehabilitation_sk',
    strategy='check',
    alias='wc_tmfgwcc_rehabilitation',
    ...
    tags=['snapshot_tmf','snapshot_curated','snapshot','wc_tmfgwcc_rehabilitation']
) }}
...
WITH source_data AS (
    SELECT
        {{ dbt_utils.generate_surrogate_key([
            ...
            'work_trial_host_employer_abn'
    ]) }} AS wc_tmfgwcc_rehabilitation_sk,
    ...
    FROM {{ source('ifnsw_claim_interface', 'wc_tmfgwcc_rehabilitation') }}
)
