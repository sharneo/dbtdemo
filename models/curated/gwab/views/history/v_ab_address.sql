...
2026-01-01      0.0                             Curated SCD2 full history view for ab_address.
                                                Source: {{ ref('ab_address_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "contact_manager", "non_business_critical", "ab_address"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('ab_address_snapshot')) }}
    FROM {{ ref('ab_address_snapshot') }}
),
