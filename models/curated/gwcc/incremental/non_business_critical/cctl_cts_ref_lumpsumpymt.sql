...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_ref_lumpsumpymt.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_ref_lumpsumpymt')) }}
    FROM {{ ref('stg_cctl_cts_ref_lumpsumpymt') }}
    {% if is_incremental() %}
