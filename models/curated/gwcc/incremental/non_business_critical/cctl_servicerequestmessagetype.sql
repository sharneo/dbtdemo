...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequestmessagetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequestmessagetype')) }}
    FROM {{ ref('stg_cctl_servicerequestmessagetype') }}
    {% if is_incremental() %}
