...
2026-01-01      0.0                             Incremental Curated model for cctl_contactdestructionstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactdestructionstatus')) }}
    FROM {{ ref('stg_cctl_contactdestructionstatus') }}
    {% if is_incremental() %}
