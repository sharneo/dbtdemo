...
2026-01-01      0.5                             Curated current-state view for cctl_paycodeapprovalreason_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_paycodeapprovalreason_ext_current')) }}
    FROM {{ ref('dt_cctl_paycodeapprovalreason_ext_current') }}
