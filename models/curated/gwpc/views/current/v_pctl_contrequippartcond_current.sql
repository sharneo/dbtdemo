...
2026-01-01      0.5                             Curated current-state view for pctl_contrequippartcond.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pctl_contrequippartcond_current')) }}
    FROM {{ ref('dt_pctl_contrequippartcond_current') }}
