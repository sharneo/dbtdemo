...
2026-01-01      0.0                             Incremental staging model for cc_endorsement.
                                                endorsement_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cc_endorsement"]
) }}
...
(
            ...
            SELECT
                ...
                CAST(data_payload:FormNumber::TEXT AS VARCHAR(64)) AS formnumber,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cc_endorsement') }}
            ...
            SELECT
                ...
                CAST($1:formnumber::TEXT AS VARCHAR(64)) AS formnumber,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS endorsement_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'createuserid',
                        'publicid',
