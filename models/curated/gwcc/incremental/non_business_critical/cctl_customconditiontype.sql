...
2026-01-01      0.0                             Incremental Curated model for cctl_customconditiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_customconditiontype')) }}
    FROM {{ ref('stg_cctl_customconditiontype') }}
    {% if is_incremental() %}
