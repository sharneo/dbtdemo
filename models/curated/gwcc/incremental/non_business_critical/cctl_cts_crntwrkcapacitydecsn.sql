...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_crntwrkcapacitydecsn.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_crntwrkcapacitydecsn')) }}
    FROM {{ ref('stg_cctl_cts_crntwrkcapacitydecsn') }}
    {% if is_incremental() %}
