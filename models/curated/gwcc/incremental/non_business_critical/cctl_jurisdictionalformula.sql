...
2026-01-01      0.0                             Incremental Curated model for cctl_jurisdictionalformula.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_jurisdictionalformula')) }}
    FROM {{ ref('stg_cctl_jurisdictionalformula') }}
    {% if is_incremental() %}
