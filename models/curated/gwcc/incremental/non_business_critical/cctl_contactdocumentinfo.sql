...
2026-01-01      0.0                             Incremental Curated model for cctl_contactdocumentinfo.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactdocumentinfo')) }}
    FROM {{ ref('stg_cctl_contactdocumentinfo') }}
    {% if is_incremental() %}
