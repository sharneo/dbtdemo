...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_injurylocation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_injurylocation')) }}
    FROM {{ ref('stg_cctl_cts_injurylocation') }}
    {% if is_incremental() %}
