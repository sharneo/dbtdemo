...
2026-01-01      0.0                             Incremental Curated model for cctl_loadercallbacktimetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_loadercallbacktimetype')) }}
    FROM {{ ref('stg_cctl_loadercallbacktimetype') }}
    {% if is_incremental() %}
