...
2026-01-01      0.0                             Incremental Curated model for cctl_claiminfocriteria.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claiminfocriteria')) }}
    FROM {{ ref('stg_cctl_claiminfocriteria') }}
    {% if is_incremental() %}
