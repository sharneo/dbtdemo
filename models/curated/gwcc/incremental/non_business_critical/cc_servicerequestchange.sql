...
2026-01-01      0.0                             Incremental Curated model for cc_servicerequestchange.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_servicerequestchange')) }}
    FROM {{ ref('stg_cc_servicerequestchange') }}
    {% if is_incremental() %}
