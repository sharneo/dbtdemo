...
2026-01-01      0.0                             Incremental staging model for cc_checkportion.
                                                checkportion_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "business_critical", "cc_checkportion"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS checkportion_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'fixedclaimamount',
                        'loadcommandid',
