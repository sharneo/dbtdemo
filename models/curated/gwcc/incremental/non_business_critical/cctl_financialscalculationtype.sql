...
2026-01-01      0.0                             Incremental Curated model for cctl_financialscalculationtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_financialscalculationtype')) }}
    FROM {{ ref('stg_cctl_financialscalculationtype') }}
    {% if is_incremental() %}
