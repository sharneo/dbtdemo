...
2026-01-01      0.0                             Curated current-state view for bctl_disbursementstatus.
                                                Source: {{ ref('bctl_disbursementstatus_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_current", "billing_centre", "non_business_critical", "bctl_disbursementstatus"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('bctl_disbursementstatus_snapshot')) }}
    FROM {{ ref('bctl_disbursementstatus_snapshot') }}
