...
2026-01-01      0.5                             Curated current-state view for cc_servicerequestinstruction.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cc_servicerequestinstruction_current')) }}
    FROM {{ ref('dt_cc_servicerequestinstruction_current') }}
