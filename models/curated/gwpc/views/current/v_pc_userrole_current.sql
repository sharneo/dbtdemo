...
2026-01-01      0.0                             Curated current-state view for pc_userrole.
                                                Source: {{ ref('pc_userrole_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_current", "policy_centre", "non_business_critical", "pc_userrole"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pc_userrole_snapshot')) }}
    FROM {{ ref('pc_userrole_snapshot') }}
