...
2026-01-01      0.0                             Incremental Curated model for ccx_servicerequestoutcome.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_servicerequestoutcome')) }}
    FROM {{ ref('stg_ccx_servicerequestoutcome') }}
    {% if is_incremental() %}
