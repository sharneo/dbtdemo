...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_nonpecuniarybenefits.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_nonpecuniarybenefits')) }}
    FROM {{ ref('stg_ccx_cts_nonpecuniarybenefits') }}
    {% if is_incremental() %}
