...
2026-01-01      0.0                             Incremental Curated model for cctl_lostwagesbenefittype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_lostwagesbenefittype')) }}
    FROM {{ ref('stg_cctl_lostwagesbenefittype') }}
    {% if is_incremental() %}
