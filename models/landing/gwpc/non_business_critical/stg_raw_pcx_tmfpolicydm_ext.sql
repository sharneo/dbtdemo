...
2026-01-01      0.0                             Incremental staging model for pcx_tmfpolicydm_ext.
                                                tmfpolicydm_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "non_business_critical", "pcx_tmfpolicydm_ext"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS tmfpolicydm_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'agencycontactemail',
                        'publicid',
