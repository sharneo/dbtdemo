...
2026-01-01      0.0                             Curated SCD2 full history view for cc_matter.
                                                Source: {{ ref('cc_matter_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "business_critical", "cc_matter"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cc_matter_snapshot')) }}
    FROM {{ ref('cc_matter_snapshot') }}
),
