...
2026-01-01      0.0                             Incremental staging model for cc_reserveline.
                                                reserveline_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cc_reserveline"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cc_reserveline') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS reserveline_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'exposureid',
                        'createuserid',
