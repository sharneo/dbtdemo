...
2026-01-01      0.5                             Dynamic Table for current-state of bc_policyperiod.
...
{{ config(
    ...
    tags=["curated", "gwbc", "billing_centre", "business_critical", "dynamic_table"]
) }}

SELECT
    {{ dbt_utils.star(from=ref('v_bc_policyperiod')) }}
FROM {{ ref('v_bc_policyperiod') }}
