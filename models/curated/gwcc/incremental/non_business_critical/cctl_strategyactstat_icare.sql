...
2026-01-01      0.0                             Incremental Curated model for cctl_strategyactstat_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_strategyactstat_icare')) }}
    FROM {{ ref('stg_cctl_strategyactstat_icare') }}
    {% if is_incremental() %}
