...
2026-01-01      0.0                             Incremental Curated model for cctl_paymentfrequencytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_paymentfrequencytype')) }}
    FROM {{ ref('stg_cctl_paymentfrequencytype') }}
    {% if is_incremental() %}
