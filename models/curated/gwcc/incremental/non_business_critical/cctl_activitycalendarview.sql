...
2026-01-01      0.0                             Incremental Curated model for cctl_activitycalendarview.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_activitycalendarview')) }}
    FROM {{ ref('stg_cctl_activitycalendarview') }}
    {% if is_incremental() %}
