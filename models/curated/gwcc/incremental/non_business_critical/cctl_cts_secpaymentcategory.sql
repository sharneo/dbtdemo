...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_secpaymentcategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_secpaymentcategory')) }}
    FROM {{ ref('stg_cctl_cts_secpaymentcategory') }}
    {% if is_incremental() %}
