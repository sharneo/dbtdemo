...
2026-01-01      0.5                             Curated current-state view for bctl_invoicepaidstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_invoicepaidstatus_current')) }}
    FROM {{ ref('dt_bctl_invoicepaidstatus_current') }}
