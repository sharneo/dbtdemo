...
2026-01-01      0.0                             Incremental Curated model for cctl_underwritingcompanytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_underwritingcompanytype')) }}
    FROM {{ ref('stg_cctl_underwritingcompanytype') }}
    {% if is_incremental() %}
