...
2026-01-01      0.0                             Incremental Curated model for ccx_servreqlinkable_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_servreqlinkable_icare')) }}
    FROM {{ ref('stg_ccx_servreqlinkable_icare') }}
    {% if is_incremental() %}
