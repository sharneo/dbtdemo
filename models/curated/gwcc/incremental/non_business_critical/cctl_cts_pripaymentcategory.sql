...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_pripaymentcategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_pripaymentcategory')) }}
    FROM {{ ref('stg_cctl_cts_pripaymentcategory') }}
    {% if is_incremental() %}
