...
2026-01-01      0.5                             Curated current-state view for bctl_acctcorrespondencetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_acctcorrespondencetype_current')) }}
    FROM {{ ref('dt_bctl_acctcorrespondencetype_current') }}
