...
2026-01-01      0.0                             Incremental Curated model for cctl_mattervenuerating.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_mattervenuerating')) }}
    FROM {{ ref('stg_cctl_mattervenuerating') }}
    {% if is_incremental() %}
