...
2026-01-01      0.0                             Incremental staging model for ccx_transactionsetuser_ext.
...
(
            ...
            FROM {{ source('gwcc', 'ccx_transactionsetuser_ext') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS transactionsetuser_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'createuserid',
                        'publicid',
