...
2026-01-01      0.5                             Curated current-state view for bctl_listbillacctexcess.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_listbillacctexcess_current')) }}
    FROM {{ ref('dt_bctl_listbillacctexcess_current') }}
