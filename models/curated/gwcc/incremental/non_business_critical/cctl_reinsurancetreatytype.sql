...
2026-01-01      0.0                             Incremental Curated model for cctl_reinsurancetreatytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_reinsurancetreatytype')) }}
    FROM {{ ref('stg_cctl_reinsurancetreatytype') }}
    {% if is_incremental() %}
