...
2026-01-01      0.0                             Incremental Curated model for cctl_archivesourcestatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_archivesourcestatus')) }}
    FROM {{ ref('stg_cctl_archivesourcestatus') }}
    {% if is_incremental() %}
