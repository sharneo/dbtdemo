...
2026-01-01      0.0                             Incremental Curated model for ccx_mspmedicaltreatment_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_mspmedicaltreatment_ext')) }}
    FROM {{ ref('stg_ccx_mspmedicaltreatment_ext') }}
    {% if is_incremental() %}
