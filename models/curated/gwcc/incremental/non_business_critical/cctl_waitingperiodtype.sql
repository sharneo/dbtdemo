...
2026-01-01      0.0                             Incremental Curated model for cctl_waitingperiodtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_waitingperiodtype')) }}
    FROM {{ ref('stg_cctl_waitingperiodtype') }}
    {% if is_incremental() %}
