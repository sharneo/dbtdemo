...
2026-01-01      0.0                             Incremental staging model for abtl_customhistorytype.
                                                customhistorytype_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["layer:stg", "platform:guidewire","module:gwab","domain:contact_manager", "criticality:non_business_critical", "entity:abtl_customhistorytype"]
) }}
...
(
            ...
            FROM {{ source('gwab', 'abtl_customhistorytype') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS customhistorytype_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'l_en_us',
                        'priority',
                        's_en_us_edg_ph',
