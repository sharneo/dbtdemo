...
2026-01-01      0.0                             Incremental Curated model for cctl_propertygroup_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_propertygroup_psc')) }}
    FROM {{ ref('stg_cctl_propertygroup_psc') }}
    {% if is_incremental() %}
