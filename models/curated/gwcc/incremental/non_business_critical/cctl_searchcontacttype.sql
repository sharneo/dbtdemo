...
2026-01-01      0.0                             Incremental Curated model for cctl_searchcontacttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_searchcontacttype')) }}
    FROM {{ ref('stg_cctl_searchcontacttype') }}
    {% if is_incremental() %}
