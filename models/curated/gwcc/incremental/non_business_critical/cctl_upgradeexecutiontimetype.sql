...
2026-01-01      0.0                             Incremental Curated model for cctl_upgradeexecutiontimetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_upgradeexecutiontimetype')) }}
    FROM {{ ref('stg_cctl_upgradeexecutiontimetype') }}
    {% if is_incremental() %}
