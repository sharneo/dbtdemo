...
2026-01-01      0.5                             Curated current-state view for pctl_crtypeofcover_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pctl_crtypeofcover_icare_current')) }}
    FROM {{ ref('dt_pctl_crtypeofcover_icare_current') }}
