...
2026-01-01      0.0                             Incremental Curated model for cctl_activitysubjectsearchtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_activitysubjectsearchtype')) }}
    FROM {{ ref('stg_cctl_activitysubjectsearchtype') }}
    {% if is_incremental() %}
