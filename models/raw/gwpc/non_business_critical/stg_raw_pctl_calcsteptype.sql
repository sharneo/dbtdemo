...
2026-01-01      0.0                             Incremental staging model for pctl_calcsteptype.
                                                calcsteptype_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "non_business_critical", "pctl_calcsteptype"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwpc', 'pctl_calcsteptype') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS calcsteptype_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        's_en_us_edg',
                        'l_en_us',
                        'priority',
