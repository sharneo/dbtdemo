...
2026-01-01      0.5                             SCD Type II view over ccx_garnisheecheckslink_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('ccx_garnisheecheckslink_icare')) }}
    FROM {{ ref('ccx_garnisheecheckslink_icare') }}
...
(
    SELECT
        *,
        coalesce(gwcbi_payload_ts_ms, updatetime) AS VALID_FROM
    FROM cte_source_data
    QUALIFY ROW_NUMBER() OVER (
        PARTITION BY ID, coalesce(gwcbi_payload_ts_ms, updatetime)
