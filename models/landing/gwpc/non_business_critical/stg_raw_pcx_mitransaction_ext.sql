...
2026-01-01      0.0                             Incremental staging model for pcx_mitransaction_ext.
                                                mitransaction_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "non_business_critical", "pcx_mitransaction_ext"]
) }}
...
(
            ...
            SELECT
                ...
                data_payload:MICostID::NUMBER AS micostid,
                ...
                'AVRO' file_type
            FROM {{ source('gwpc', 'pcx_mitransaction_ext') }}
            WHERE REGEXP_SUBSTR(metadata_file_name, '[^.]+$') = 'avro'
            UNION ALL 
            SELECT
                $1:loadcommandid::NUMBER AS loadcommandid,
                CAST($1:publicid::TEXT AS VARCHAR(64)) AS publicid,
                $1:micostid::NUMBER AS micostid,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS mitransaction_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'publicid',
                        'micostid',
