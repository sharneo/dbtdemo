...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_ref_wbolsecrules.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_ref_wbolsecrules')) }}
    FROM {{ ref('stg_ccx_cts_ref_wbolsecrules') }}
    {% if is_incremental() %}
