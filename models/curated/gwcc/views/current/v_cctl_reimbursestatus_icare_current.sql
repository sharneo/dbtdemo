...
2026-01-01      0.5                             Curated current-state view for cctl_reimbursestatus_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_reimbursestatus_icare_current')) }}
    FROM {{ ref('dt_cctl_reimbursestatus_icare_current') }}
