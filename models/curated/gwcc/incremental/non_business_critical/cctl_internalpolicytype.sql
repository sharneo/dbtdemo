...
2026-01-01      0.0                             Incremental Curated model for cctl_internalpolicytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_internalpolicytype')) }}
    FROM {{ ref('stg_cctl_internalpolicytype') }}
    {% if is_incremental() %}
