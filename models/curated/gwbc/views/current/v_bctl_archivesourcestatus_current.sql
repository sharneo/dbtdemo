...
2026-01-01      0.5                             Curated current-state view for bctl_archivesourcestatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_archivesourcestatus_current')) }}
    FROM {{ ref('dt_bctl_archivesourcestatus_current') }}
