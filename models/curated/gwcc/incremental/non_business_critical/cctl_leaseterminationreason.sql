...
2026-01-01      0.0                             Incremental Curated model for cctl_leaseterminationreason.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_leaseterminationreason')) }}
    FROM {{ ref('stg_cctl_leaseterminationreason') }}
    {% if is_incremental() %}
