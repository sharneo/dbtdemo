...
2026-01-01      0.0                             Incremental Curated model for cctl_dpplatformtype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_dpplatformtype_ext')) }}
    FROM {{ ref('stg_cctl_dpplatformtype_ext') }}
    {% if is_incremental() %}
