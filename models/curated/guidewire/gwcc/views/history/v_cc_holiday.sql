...
2026-01-01      0.0                             Curated SCD2 full history view for cc_holiday.
                                                Source: {{ ref('cc_holiday_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "non_business_critical", "cc_holiday"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cc_holiday_snapshot')) }}
    FROM {{ ref('cc_holiday_snapshot') }}
),
