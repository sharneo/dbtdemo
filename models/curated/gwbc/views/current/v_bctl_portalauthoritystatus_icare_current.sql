...
2026-01-01      0.5                             Curated current-state view for bctl_portalauthoritystatus_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_portalauthoritystatus_icare_current')) }}
    FROM {{ ref('dt_bctl_portalauthoritystatus_icare_current') }}
