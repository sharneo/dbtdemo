...
2026-01-01      0.0                             Curated SCD2 full history view for abtl_state.
                                                Source: {{ ref('abtl_state_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "contact_manager", "non_business_critical", "abtl_state"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('abtl_state_snapshot')) }}
    FROM {{ ref('abtl_state_snapshot') }}
),
