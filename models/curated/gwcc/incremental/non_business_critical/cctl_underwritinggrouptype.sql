...
2026-01-01      0.0                             Incremental Curated model for cctl_underwritinggrouptype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_underwritinggrouptype')) }}
    FROM {{ ref('stg_cctl_underwritinggrouptype') }}
    {% if is_incremental() %}
