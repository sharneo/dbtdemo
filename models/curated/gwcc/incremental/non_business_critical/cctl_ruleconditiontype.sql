...
2026-01-01      0.0                             Incremental Curated model for cctl_ruleconditiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_ruleconditiontype')) }}
    FROM {{ ref('stg_cctl_ruleconditiontype') }}
    {% if is_incremental() %}
