...
2026-01-01      0.0                             Incremental Curated model for cctl_bivalidationalerttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_bivalidationalerttype')) }}
    FROM {{ ref('stg_cctl_bivalidationalerttype') }}
    {% if is_incremental() %}
