...
2026-01-01      0.0                             Incremental Curated model for cctl_daterangechoicetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_daterangechoicetype')) }}
    FROM {{ ref('stg_cctl_daterangechoicetype') }}
    {% if is_incremental() %}
