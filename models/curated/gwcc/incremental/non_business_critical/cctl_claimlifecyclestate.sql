...
2026-01-01      0.0                             Incremental Curated model for cctl_claimlifecyclestate.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_claimlifecyclestate')) }}
    FROM {{ ref('stg_cctl_claimlifecyclestate') }}
    {% if is_incremental() %}
