...
2026-01-01      0.0                             Incremental Curated model for cctl_rescontributingfactors.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_rescontributingfactors')) }}
    FROM {{ ref('stg_cctl_rescontributingfactors') }}
    {% if is_incremental() %}
