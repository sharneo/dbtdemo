...
2026-01-01      0.0                             Incremental Curated model for cctl_servicereqinvoicestatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicereqinvoicestatus')) }}
    FROM {{ ref('stg_cctl_servicereqinvoicestatus') }}
    {% if is_incremental() %}
