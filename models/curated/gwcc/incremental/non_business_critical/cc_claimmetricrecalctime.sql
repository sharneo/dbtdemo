...
2026-01-01      0.0                             Incremental Curated model for cc_claimmetricrecalctime.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_claimmetricrecalctime')) }}
    FROM {{ ref('stg_cc_claimmetricrecalctime') }}
    {% if is_incremental() %}
