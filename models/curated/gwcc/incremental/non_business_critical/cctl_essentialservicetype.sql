...
2026-01-01      0.0                             Incremental Curated model for cctl_essentialservicetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_essentialservicetype')) }}
    FROM {{ ref('stg_cctl_essentialservicetype') }}
    {% if is_incremental() %}
