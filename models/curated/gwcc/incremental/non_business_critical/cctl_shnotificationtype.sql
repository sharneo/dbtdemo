...
2026-01-01      0.0                             Incremental Curated model for cctl_shnotificationtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_shnotificationtype')) }}
    FROM {{ ref('stg_cctl_shnotificationtype') }}
    {% if is_incremental() %}
