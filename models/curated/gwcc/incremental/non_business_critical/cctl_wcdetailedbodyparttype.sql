...
2026-01-01      0.0                             Incremental Curated model for cctl_wcdetailedbodyparttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wcdetailedbodyparttype')) }}
    FROM {{ ref('stg_cctl_wcdetailedbodyparttype') }}
    {% if is_incremental() %}
