...
2026-01-01      0.0                             Curated current-state view for pc_organization.
                                                Source: {{ ref('pc_organization_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_current", "policy_centre", "non_business_critical", "pc_organization"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pc_organization_snapshot')) }}
    FROM {{ ref('pc_organization_snapshot') }}
