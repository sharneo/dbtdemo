...
2026-01-01      0.0                             Incremental Curated model for cctl_datagenstatustype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_datagenstatustype')) }}
    FROM {{ ref('stg_cctl_datagenstatustype') }}
    {% if is_incremental() %}
