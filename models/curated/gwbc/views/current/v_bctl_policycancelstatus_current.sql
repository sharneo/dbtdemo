...
2026-01-01      0.5                             Curated current-state view for bctl_policycancelstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_bctl_policycancelstatus_current')) }}
    FROM {{ ref('dt_bctl_policycancelstatus_current') }}
