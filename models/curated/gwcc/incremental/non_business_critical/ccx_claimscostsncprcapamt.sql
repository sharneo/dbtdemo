...
2026-01-01      0.0                             Incremental Curated model for ccx_claimscostsncprcapamt.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_claimscostsncprcapamt')) }}
    FROM {{ ref('stg_ccx_claimscostsncprcapamt') }}
    {% if is_incremental() %}
