...
2026-01-01      0.5                             Curated current-state view for cc_assignableforkeydatadist.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cc_assignableforkeydatadist_current')) }}
    FROM {{ ref('dt_cc_assignableforkeydatadist_current') }}
