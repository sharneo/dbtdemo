...
2026-01-01      0.0                             Curated SCD2 full history view for bctl_plan.
                                                Source: {{ ref('bctl_plan_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "billing_centre", "non_business_critical", "bctl_plan"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('bctl_plan_snapshot')) }}
    FROM {{ ref('bctl_plan_snapshot') }}
),
