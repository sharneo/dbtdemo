...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_weeklyentitlementrate.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_weeklyentitlementrate')) }}
    FROM {{ ref('stg_cctl_cts_weeklyentitlementrate') }}
    {% if is_incremental() %}
