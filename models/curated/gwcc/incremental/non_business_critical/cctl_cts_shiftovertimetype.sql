...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_shiftovertimetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_shiftovertimetype')) }}
    FROM {{ ref('stg_cctl_cts_shiftovertimetype') }}
    {% if is_incremental() %}
