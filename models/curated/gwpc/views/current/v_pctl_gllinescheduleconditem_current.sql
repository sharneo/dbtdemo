...
2026-01-01      0.5                             Curated current-state view for pctl_gllinescheduleconditem.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pctl_gllinescheduleconditem_current')) }}
    FROM {{ ref('dt_pctl_gllinescheduleconditem_current') }}
