...
2026-01-01      0.0                             Curated SCD2 full history view for pctl_state.
                                                Source: {{ ref('pctl_state_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "policy_centre", "business_critical", "pctl_state"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pctl_state_snapshot')) }}
    FROM {{ ref('pctl_state_snapshot') }}
),
