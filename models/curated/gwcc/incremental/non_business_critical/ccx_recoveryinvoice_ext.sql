...
2026-01-01      0.0                             Incremental Curated model for ccx_recoveryinvoice_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_recoveryinvoice_ext')) }}
    FROM {{ ref('stg_ccx_recoveryinvoice_ext') }}
    {% if is_incremental() %}
