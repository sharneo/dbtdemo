...
2026-01-01      0.0                             Incremental Curated model for cctl_approvalroutingtype_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_approvalroutingtype_psc')) }}
    FROM {{ ref('stg_cctl_approvalroutingtype_psc') }}
    {% if is_incremental() %}
