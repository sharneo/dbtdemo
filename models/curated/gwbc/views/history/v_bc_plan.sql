...
2026-01-01      0.0                             Curated SCD2 full history view for bc_plan.
                                                Source: {{ ref('bc_plan_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "billing_centre", "business_critical", "bc_plan"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('bc_plan_snapshot')) }}
    FROM {{ ref('bc_plan_snapshot') }}
),
