...
2026-01-01      0.0                             Incremental Curated model for cctl_reasonforexempt_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_reasonforexempt_ext')) }}
    FROM {{ ref('stg_cctl_reasonforexempt_ext') }}
    {% if is_incremental() %}
