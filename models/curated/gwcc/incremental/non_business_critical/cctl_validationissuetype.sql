...
2026-01-01      0.0                             Incremental Curated model for cctl_validationissuetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_validationissuetype')) }}
    FROM {{ ref('stg_cctl_validationissuetype') }}
    {% if is_incremental() %}
