...
2026-01-01      0.0                             Incremental Curated model for cctl_exposurereopenedreason.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_exposurereopenedreason')) }}
    FROM {{ ref('stg_cctl_exposurereopenedreason') }}
    {% if is_incremental() %}
