...
2026-01-01      0.0                             Incremental Curated model for cc_rectaccounttransaction.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_rectaccounttransaction')) }}
    FROM {{ ref('stg_cc_rectaccounttransaction') }}
    {% if is_incremental() %}
