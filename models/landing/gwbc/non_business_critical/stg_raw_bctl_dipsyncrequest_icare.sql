...
2026-01-01      0.0                             Incremental staging model for bctl_dipsyncrequest_icare.
                                                dipsyncrequest_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_billing_centre", "billing_centre", "non_business_critical", "bctl_dipsyncrequest_icare"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwbc', 'bctl_dipsyncrequest_icare') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS dipsyncrequest_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'l_en_us',
                        'priority',
                        'typecode',
