...
2026-01-01      0.0                             Incremental staging model for pcx_bpcspxferworkitem_ext.
                                                bpcspxferworkitem_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_policy_centre", "policy_centre", "non_business_critical", "pcx_bpcspxferworkitem_ext"]
) }}
...
(
            ...
            SELECT
                ...
                data_payload:PolicyCSPXferID::NUMBER AS policycspxferid,
                ...
                'AVRO' file_type
            FROM {{ source('gwpc', 'pcx_bpcspxferworkitem_ext') }}
            ...
            SELECT
                ...
                $1:policycspxferid::NUMBER AS policycspxferid,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS bpcspxferworkitem_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'processhistoryid',
                        'publicid',
