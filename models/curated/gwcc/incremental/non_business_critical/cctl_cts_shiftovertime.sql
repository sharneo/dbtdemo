...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_shiftovertime.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_shiftovertime')) }}
    FROM {{ ref('stg_cctl_cts_shiftovertime') }}
    {% if is_incremental() %}
