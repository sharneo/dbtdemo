...
2026-01-01      0.0                             Incremental Curated model for ccx_planstratrevparties_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_planstratrevparties_icare')) }}
    FROM {{ ref('stg_ccx_planstratrevparties_icare') }}
    {% if is_incremental() %}
