...
2026-01-01      0.5                             Curated current-state view for abtl_paymentmethod_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_abtl_paymentmethod_icare_current')) }}
    FROM {{ ref('dt_abtl_paymentmethod_icare_current') }}
