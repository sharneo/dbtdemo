...
2026-01-01      0.5                             Dynamic Table for current-state of cctl_mismatchpresentedcheckstatus_ext.
...
{{ config(
    ...
    tags=["curated", "gwcc", "claim_centre", "non_business_critical", "dynamic_table"]
) }}

SELECT
    {{ dbt_utils.star(from=ref('v_cctl_mismatchpresentedcheckstatus_ext')) }}
FROM {{ ref('v_cctl_mismatchpresentedcheckstatus_ext') }}
