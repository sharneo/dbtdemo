...
2026-01-01      0.0                             Incremental staging model for bctl_billinginstruction.
                                                billinginstruction_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_billing_centre", "billing_centre", "non_business_critical", "bctl_billinginstruction"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwbc', 'bctl_billinginstruction') }}
