...
2026-01-01      0.0                             Incremental Curated model for cctl_servicereqstmtlineitemcat.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicereqstmtlineitemcat')) }}
    FROM {{ ref('stg_cctl_servicereqstmtlineitemcat') }}
    {% if is_incremental() %}
