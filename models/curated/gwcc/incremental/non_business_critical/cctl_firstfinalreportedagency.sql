...
2026-01-01      0.0                             Incremental Curated model for cctl_firstfinalreportedagency.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_firstfinalreportedagency')) }}
    FROM {{ ref('stg_cctl_firstfinalreportedagency') }}
    {% if is_incremental() %}
