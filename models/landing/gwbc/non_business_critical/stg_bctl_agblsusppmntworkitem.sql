...
2026-01-01      0.0                             Incremental staging model for bctl_agblsusppmntworkitem.
                                                agblsusppmntworkitem_sk: Entity identity surrogate key on PK ('id')
...
(
            ...
            FROM {{ source('gwbc', 'bctl_agblsusppmntworkitem') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS agblsusppmntworkitem_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        's_en_us_edg',
                        'l_en_us',
                        'priority',
