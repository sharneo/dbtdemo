...
2026-01-01      0.0                             Curated SCD2 full history view for pctl_rule.
                                                Source: {{ ref('pctl_rule_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "policy_centre", "non_business_critical", "pctl_rule"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pctl_rule_snapshot')) }}
    FROM {{ ref('pctl_rule_snapshot') }}
),
