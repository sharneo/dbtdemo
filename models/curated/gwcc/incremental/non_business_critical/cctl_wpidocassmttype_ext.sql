...
2026-01-01      0.0                             Incremental Curated model for cctl_wpidocassmttype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wpidocassmttype_ext')) }}
    FROM {{ ref('stg_cctl_wpidocassmttype_ext') }}
    {% if is_incremental() %}
