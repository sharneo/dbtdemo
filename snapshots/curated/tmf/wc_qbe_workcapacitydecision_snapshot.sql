{% snapshot wc_qbe_workcapacitydecision_snapshot %}
...
2026.01.11      0.0                             This Creates a  View for the Snapshot for the Table wc_qbe_workcapacitydecision_snapshot . 
...
{{ config(
    ...
    unique_key='wc_qbe_workcapacitydecision_sk',
    strategy='check',
    alias='wc_qbe_workcapacitydecision',
    ...
    tags=['snapshot_tmf','snapshot_curated','snapshot','wc_qbe_workcapacitydecision']
) }}
...
WITH source_data AS (
    ...
    FROM {{ source('ifnsw_claim_interface', 'wc_qbe_workcapacitydecision') }}
)
