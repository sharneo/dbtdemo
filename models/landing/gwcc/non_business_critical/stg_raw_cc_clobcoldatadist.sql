...
2026-01-01      0.0                             Incremental staging model for cc_clobcoldatadist.
                                                clobcoldatadist_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cc_clobcoldatadist"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS clobcoldatadist_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'averagelength',
                        'publicid',
                        'minimumlength',
