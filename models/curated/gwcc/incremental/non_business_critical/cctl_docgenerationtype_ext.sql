...
2026-01-01      0.0                             Incremental Curated model for cctl_docgenerationtype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_docgenerationtype_ext')) }}
    FROM {{ ref('stg_cctl_docgenerationtype_ext') }}
    {% if is_incremental() %}
