...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_ref_piaweindexation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_ref_piaweindexation')) }}
    FROM {{ ref('stg_cctl_cts_ref_piaweindexation') }}
    {% if is_incremental() %}
