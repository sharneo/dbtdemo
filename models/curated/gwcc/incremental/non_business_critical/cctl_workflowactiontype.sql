...
2026-01-01      0.0                             Incremental Curated model for cctl_workflowactiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_workflowactiontype')) }}
    FROM {{ ref('stg_cctl_workflowactiontype') }}
    {% if is_incremental() %}
