...
2026-01-01      0.0                             Incremental Curated model for cctl_datagenactiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_datagenactiontype')) }}
    FROM {{ ref('stg_cctl_datagenactiontype') }}
    {% if is_incremental() %}
