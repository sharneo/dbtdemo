...
2026-01-01      0.0                             Incremental staging model for cctl_mismatchpresentedcheckstatus_ext.
                                                mismatchpresentedcheckstatus_ext_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cctl_mismatchpresentedcheckstatus_ext"]
) }}
...
(
            ...
            SELECT
                ...
                data_payload:S_en_US_EDG_PH::NUMBER AS s_en_us_edg_ph,
                ...
                CAST(data_payload:L_en_US_EDG_PH::TEXT AS VARCHAR(256)) AS l_en_us_edg_ph,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cctl_mismatchpresentedcheckstatus_ext') }}
            WHERE REGEXP_SUBSTR(metadata_file_name, '[^.]+$') = 'avro'
            UNION ALL 
            SELECT
                CAST($1:l_en_us::TEXT AS VARCHAR(256)) AS l_en_us,
                $1:priority::NUMBER AS priority,
                $1:s_en_us_edg_ph::NUMBER AS s_en_us_edg_ph,
                ...
                CAST($1:l_en_us_edg_ph::TEXT AS VARCHAR(256)) AS l_en_us_edg_ph,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS mismatchpresentedcheckstatus_ext_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'l_en_us',
                        'priority',
                        's_en_us_edg_ph',
