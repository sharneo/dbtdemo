...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_mdcrdpndtlimitype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_mdcrdpndtlimitype')) }}
    FROM {{ ref('stg_cctl_cts_mdcrdpndtlimitype') }}
    {% if is_incremental() %}
