...
2026-01-01      0.0                             Curated SCD2 full history view for pc_history.
                                                Source: {{ ref('pc_history_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "policy_centre", "non_business_critical", "pc_history"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pc_history_snapshot')) }}
    FROM {{ ref('pc_history_snapshot') }}
),
