...
2026-01-01      0.5                             Curated current-state view for ccx_documentfollower_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_ccx_documentfollower_icare_current')) }}
    FROM {{ ref('dt_ccx_documentfollower_icare_current') }}
