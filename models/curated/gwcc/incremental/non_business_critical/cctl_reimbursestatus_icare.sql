...
2026-01-01      0.0                             Incremental Curated model for cctl_reimbursestatus_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_reimbursestatus_icare')) }}
    FROM {{ ref('stg_cctl_reimbursestatus_icare') }}
    {% if is_incremental() %}
