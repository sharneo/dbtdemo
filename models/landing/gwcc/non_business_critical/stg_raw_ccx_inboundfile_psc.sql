...
2026-01-01      0.0                             Incremental staging model for ccx_inboundfile_psc.
                                                inboundfile_psc_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_inboundfile_psc"]
) }}
...
(
            ...
            SELECT
                ...
                CAST(data_payload:FileLoader_Ext::TEXT AS VARCHAR(255)) AS fileloader_ext,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_inboundfile_psc') }}
            ...
            SELECT
                ...
                CAST($1:fileloader_ext::TEXT AS VARCHAR(255)) AS fileloader_ext,
