...
2026-01-01      0.0                             Incremental Curated model for ccx_paygsupplierpayer_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_paygsupplierpayer_icare')) }}
    FROM {{ ref('stg_ccx_paygsupplierpayer_icare') }}
    {% if is_incremental() %}
