...
2026-01-01      0.0                             Incremental Curated model for cctl_taxfilingstatustype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_taxfilingstatustype')) }}
    FROM {{ ref('stg_cctl_taxfilingstatustype') }}
    {% if is_incremental() %}
