...
2026-01-01      0.0                             Incremental Curated model for cctl_classificationcondition.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_classificationcondition')) }}
    FROM {{ ref('stg_cctl_classificationcondition') }}
    {% if is_incremental() %}
