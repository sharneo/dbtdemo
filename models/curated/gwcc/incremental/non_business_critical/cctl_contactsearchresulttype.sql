...
2026-01-01      0.0                             Incremental Curated model for cctl_contactsearchresulttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactsearchresulttype')) }}
    FROM {{ ref('stg_cctl_contactsearchresulttype') }}
    {% if is_incremental() %}
