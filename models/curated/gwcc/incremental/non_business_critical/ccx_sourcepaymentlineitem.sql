...
2026-01-01      0.0                             Incremental Curated model for ccx_sourcepaymentlineitem.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_sourcepaymentlineitem')) }}
    FROM {{ ref('stg_ccx_sourcepaymentlineitem') }}
    {% if is_incremental() %}
