...
2026-01-01      0.5                             Curated current-state view for ccx_cts_ref_retirementage.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_ccx_cts_ref_retirementage_current')) }}
    FROM {{ ref('dt_ccx_cts_ref_retirementage_current') }}
