...
2026-01-01      0.0                             Incremental Curated model for ccx_sourcepiaweedgetable.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_sourcepiaweedgetable')) }}
    FROM {{ ref('stg_ccx_sourcepiaweedgetable') }}
    {% if is_incremental() %}
