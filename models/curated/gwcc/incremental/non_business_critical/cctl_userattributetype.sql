...
2026-01-01      0.0                             Incremental Curated model for cctl_userattributetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_userattributetype')) }}
    FROM {{ ref('stg_cctl_userattributetype') }}
    {% if is_incremental() %}
