...
2026-01-01      0.0                             Incremental Curated model for cctl_vehiclemanufacturer.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_vehiclemanufacturer')) }}
    FROM {{ ref('stg_cctl_vehiclemanufacturer') }}
    {% if is_incremental() %}
