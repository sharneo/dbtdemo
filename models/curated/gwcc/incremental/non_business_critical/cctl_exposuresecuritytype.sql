...
2026-01-01      0.0                             Incremental Curated model for cctl_exposuresecuritytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_exposuresecuritytype')) }}
    FROM {{ ref('stg_cctl_exposuresecuritytype') }}
    {% if is_incremental() %}
