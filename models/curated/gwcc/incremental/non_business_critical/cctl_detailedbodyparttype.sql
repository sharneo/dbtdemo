...
2026-01-01      0.0                             Incremental Curated model for cctl_detailedbodyparttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_detailedbodyparttype')) }}
    FROM {{ ref('stg_cctl_detailedbodyparttype') }}
    {% if is_incremental() %}
