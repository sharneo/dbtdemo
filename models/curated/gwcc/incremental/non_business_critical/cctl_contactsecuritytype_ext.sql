...
2026-01-01      0.0                             Incremental Curated model for cctl_contactsecuritytype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactsecuritytype_ext')) }}
    FROM {{ ref('stg_cctl_contactsecuritytype_ext') }}
    {% if is_incremental() %}
