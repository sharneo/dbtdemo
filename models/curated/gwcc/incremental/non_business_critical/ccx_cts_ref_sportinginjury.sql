...
2026-01-01      0.0                             Incremental Curated model for ccx_cts_ref_sportinginjury.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_cts_ref_sportinginjury')) }}
    FROM {{ ref('stg_ccx_cts_ref_sportinginjury') }}
    {% if is_incremental() %}
