...
2026-01-01      0.0                             Incremental Curated model for cctl_archivefinalstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_archivefinalstatus')) }}
    FROM {{ ref('stg_cctl_archivefinalstatus') }}
    {% if is_incremental() %}
