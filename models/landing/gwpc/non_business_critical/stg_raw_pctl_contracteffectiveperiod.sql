...
2026-01-01      0.0                             Incremental staging model for pctl_contracteffectiveperiod.
                                                contracteffectiveperiod_sk: Entity identity surrogate key on PK ('id')
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwpc', 'pctl_contracteffectiveperiod') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS contracteffectiveperiod_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        's_en_us_edg',
                        'l_en_us',
                        'priority',
