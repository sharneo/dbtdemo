...
2026-01-01      0.0                             Incremental Curated model for cctl_rulecontextdefinitionkey.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_rulecontextdefinitionkey')) }}
    FROM {{ ref('stg_cctl_rulecontextdefinitionkey') }}
    {% if is_incremental() %}
