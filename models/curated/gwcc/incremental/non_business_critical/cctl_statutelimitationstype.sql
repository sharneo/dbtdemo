...
2026-01-01      0.0                             Incremental Curated model for cctl_statutelimitationstype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_statutelimitationstype')) }}
    FROM {{ ref('stg_cctl_statutelimitationstype') }}
    {% if is_incremental() %}
