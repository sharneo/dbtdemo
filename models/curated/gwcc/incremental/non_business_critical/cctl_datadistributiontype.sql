...
2026-01-01      0.0                             Incremental Curated model for cctl_datadistributiontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_datadistributiontype')) }}
    FROM {{ ref('stg_cctl_datadistributiontype') }}
    {% if is_incremental() %}
