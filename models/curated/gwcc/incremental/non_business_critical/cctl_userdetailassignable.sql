...
2026-01-01      0.0                             Incremental Curated model for cctl_userdetailassignable.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_userdetailassignable')) }}
    FROM {{ ref('stg_cctl_userdetailassignable') }}
    {% if is_incremental() %}
