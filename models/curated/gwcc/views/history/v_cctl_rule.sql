...
2026-01-01      0.0                             Curated SCD2 full history view for cctl_rule.
                                                Source: {{ ref('cctl_rule_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "non_business_critical", "cctl_rule"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cctl_rule_snapshot')) }}
    FROM {{ ref('cctl_rule_snapshot') }}
),
