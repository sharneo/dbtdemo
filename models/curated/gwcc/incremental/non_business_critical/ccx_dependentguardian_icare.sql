...
2026-01-01      0.0                             Incremental Curated model for ccx_dependentguardian_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_dependentguardian_icare')) }}
    FROM {{ ref('stg_ccx_dependentguardian_icare') }}
    {% if is_incremental() %}
