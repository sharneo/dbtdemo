...
2026-01-01      0.0                             Incremental Curated model for cctl_workflowtriggerkey.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_workflowtriggerkey')) }}
    FROM {{ ref('stg_cctl_workflowtriggerkey') }}
    {% if is_incremental() %}
