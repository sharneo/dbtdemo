...
2026-01-01      0.5                             Curated current-state view for abtl_stateabbreviation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_abtl_stateabbreviation_current')) }}
    FROM {{ ref('dt_abtl_stateabbreviation_current') }}
