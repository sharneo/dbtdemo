{% snapshot wc_aal_payments_snapshot %}
...
2026.01.11      0.0                             This Creates a  View for the Snapshot for the Table wc_aal_payments_snapshot . 
...
{{ config(
    ...
    unique_key='wc_aal_payments_sk',
    strategy='check',
    alias='wc_aal_payments',
    ...
    tags=['snapshot_tmf','snapshot_curated','snapshot','wc_aal_payments']
) }}
...
WITH source_data AS (
    ...
    FROM {{ source('ifnsw_claim_interface', 'wc_aal_payments') }}
)
