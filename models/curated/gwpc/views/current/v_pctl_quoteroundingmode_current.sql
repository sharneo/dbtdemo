...
2026-01-01      0.5                             Curated current-state view for pctl_quoteroundingmode.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pctl_quoteroundingmode_current')) }}
    FROM {{ ref('dt_pctl_quoteroundingmode_current') }}
