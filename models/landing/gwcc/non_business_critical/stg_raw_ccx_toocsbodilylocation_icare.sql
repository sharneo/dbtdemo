...
2026-01-01      0.0                             Incremental staging model for ccx_toocsbodilylocation_icare.
                                                toocsbodilylocation_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_toocsbodilylocation_icare"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_toocsbodilylocation_icare') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS toocsbodilylocation_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
