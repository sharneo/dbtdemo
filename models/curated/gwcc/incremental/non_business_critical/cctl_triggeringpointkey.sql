...
2026-01-01      0.0                             Incremental Curated model for cctl_triggeringpointkey.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_triggeringpointkey')) }}
    FROM {{ ref('stg_cctl_triggeringpointkey') }}
    {% if is_incremental() %}
