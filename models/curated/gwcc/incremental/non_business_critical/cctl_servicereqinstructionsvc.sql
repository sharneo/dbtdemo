...
2026-01-01      0.0                             Incremental Curated model for cctl_servicereqinstructionsvc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicereqinstructionsvc')) }}
    FROM {{ ref('stg_cctl_servicereqinstructionsvc') }}
    {% if is_incremental() %}
