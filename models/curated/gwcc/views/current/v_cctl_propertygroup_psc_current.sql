...
2026-01-01      0.5                             Curated current-state view for cctl_propertygroup_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_propertygroup_psc_current')) }}
    FROM {{ ref('dt_cctl_propertygroup_psc_current') }}
