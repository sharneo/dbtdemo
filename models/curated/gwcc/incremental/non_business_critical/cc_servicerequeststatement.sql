...
2026-01-01      0.0                             Incremental Curated model for cc_servicerequeststatement.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_servicerequeststatement')) }}
    FROM {{ ref('stg_cc_servicerequeststatement') }}
    {% if is_incremental() %}
