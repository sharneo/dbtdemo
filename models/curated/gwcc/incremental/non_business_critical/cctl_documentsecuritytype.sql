...
2026-01-01      0.0                             Incremental Curated model for cctl_documentsecuritytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_documentsecuritytype')) }}
    FROM {{ ref('stg_cctl_documentsecuritytype') }}
    {% if is_incremental() %}
