...
2026-01-01      0.0                             Incremental Curated model for cctl_datefieldstosearchtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_datefieldstosearchtype')) }}
    FROM {{ ref('stg_cctl_datefieldstosearchtype') }}
    {% if is_incremental() %}
