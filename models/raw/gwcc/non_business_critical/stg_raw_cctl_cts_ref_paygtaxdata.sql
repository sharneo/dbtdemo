...
2026-01-01      0.0                             Incremental staging model for cctl_cts_ref_paygtaxdata.
                                                cts_ref_paygtaxdata_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cctl_cts_ref_paygtaxdata"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cctl_cts_ref_paygtaxdata') }}
