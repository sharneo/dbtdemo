...
2026-01-01      0.0                             Incremental Curated model for cctl_riagreementstatus.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_riagreementstatus')) }}
    FROM {{ ref('stg_cctl_riagreementstatus') }}
    {% if is_incremental() %}
