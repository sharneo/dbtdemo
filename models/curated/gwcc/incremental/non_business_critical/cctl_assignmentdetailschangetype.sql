...
2026-01-01      0.0                             Incremental Curated model for cctl_assignmentdetailschangetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_assignmentdetailschangetype')) }}
    FROM {{ ref('stg_cctl_assignmentdetailschangetype') }}
    {% if is_incremental() %}
