...
2026-01-01      0.0                             Incremental Curated model for cctl_checkhandlinginstructions.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_checkhandlinginstructions')) }}
    FROM {{ ref('stg_cctl_checkhandlinginstructions') }}
    {% if is_incremental() %}
