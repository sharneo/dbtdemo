...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_payggnrltxparameters.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_payggnrltxparameters')) }}
    FROM {{ ref('stg_cctl_cts_payggnrltxparameters') }}
    {% if is_incremental() %}
