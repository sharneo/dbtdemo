...
2026-01-01      0.0                             Incremental staging model for cc_specialistservice.
                                                specialistservice_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "business_critical", "cc_specialistservice"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cc_specialistservice') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS specialistservice_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'createuserid',
                        'publicid',
                        'active',
