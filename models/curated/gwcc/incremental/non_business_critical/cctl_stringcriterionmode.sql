...
2026-01-01      0.0                             Incremental Curated model for cctl_stringcriterionmode.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_stringcriterionmode')) }}
    FROM {{ ref('stg_cctl_stringcriterionmode') }}
    {% if is_incremental() %}
