...
2026-01-01      0.0                             Incremental Curated model for cctl_largelossnotificationstat.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_largelossnotificationstat')) }}
    FROM {{ ref('stg_cctl_largelossnotificationstat') }}
    {% if is_incremental() %}
