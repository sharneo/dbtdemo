...
2026-01-01      0.0                             Incremental Curated model for ccx_secondarysurgery_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_secondarysurgery_ext')) }}
    FROM {{ ref('stg_ccx_secondarysurgery_ext') }}
    {% if is_incremental() %}
