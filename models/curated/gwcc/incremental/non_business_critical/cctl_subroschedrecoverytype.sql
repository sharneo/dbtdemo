...
2026-01-01      0.0                             Incremental Curated model for cctl_subroschedrecoverytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_subroschedrecoverytype')) }}
    FROM {{ ref('stg_cctl_subroschedrecoverytype') }}
    {% if is_incremental() %}
