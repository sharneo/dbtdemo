...
2026-01-01      0.0                             Incremental staging model for cc_servicerequestdocumentlink.
                                                servicerequestdocumentlink_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cc_servicerequestdocumentlink"]
) }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS servicerequestdocumentlink_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'publicid',
                        'servicerequestid',
                        'documentid',
