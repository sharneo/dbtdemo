...
2026-01-01      0.0                             Curated SCD2 full history view for pctl_zone.
                                                Source: {{ ref('pctl_zone_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "policy_centre", "non_business_critical", "pctl_zone"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pctl_zone_snapshot')) }}
    FROM {{ ref('pctl_zone_snapshot') }}
),
