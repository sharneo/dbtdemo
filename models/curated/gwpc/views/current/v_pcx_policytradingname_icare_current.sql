...
2026-01-01      0.0                             Curated current-state view for pcx_policytradingname_icare.
                                                Source: {{ ref('pcx_policytradingname_icare_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_current", "policy_centre", "non_business_critical", "pcx_policytradingname_icare"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('pcx_policytradingname_icare_snapshot')) }}
    FROM {{ ref('pcx_policytradingname_icare_snapshot') }}
