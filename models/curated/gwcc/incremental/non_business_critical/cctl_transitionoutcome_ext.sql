...
2026-01-01      0.0                             Incremental Curated model for cctl_transitionoutcome_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_transitionoutcome_ext')) }}
    FROM {{ ref('stg_cctl_transitionoutcome_ext') }}
    {% if is_incremental() %}
