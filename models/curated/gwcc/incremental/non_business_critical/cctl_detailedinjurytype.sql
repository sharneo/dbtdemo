...
2026-01-01      0.0                             Incremental Curated model for cctl_detailedinjurytype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_detailedinjurytype')) }}
    FROM {{ ref('stg_cctl_detailedinjurytype') }}
    {% if is_incremental() %}
