...
2026-01-01      0.0                             Incremental Curated model for cctl_adjudicativedomain.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_adjudicativedomain')) }}
    FROM {{ ref('stg_cctl_adjudicativedomain') }}
    {% if is_incremental() %}
