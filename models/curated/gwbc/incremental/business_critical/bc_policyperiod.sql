...
2026-01-01      0.0                             Incremental Curated model for bc_policyperiod.
...
{{ config(
    ...
    tags=["curated", "gwbc", "billing_centre", "business_critical"]
) }}

WITH cte_source_data AS 
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_bc_policyperiod')) }}
    FROM {{ ref('stg_bc_policyperiod') }}
    {% if is_incremental() %}
