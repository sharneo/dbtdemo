...
2026-01-01      0.0                             Incremental Curated model for cctl_fretxtclmsrchnamesrchtyp.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_fretxtclmsrchnamesrchtyp')) }}
    FROM {{ ref('stg_cctl_fretxtclmsrchnamesrchtyp') }}
    {% if is_incremental() %}
