...
2026-01-01      0.0                             Incremental Curated model for cctl_reviewservicetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_reviewservicetype')) }}
    FROM {{ ref('stg_cctl_reviewservicetype') }}
    {% if is_incremental() %}
