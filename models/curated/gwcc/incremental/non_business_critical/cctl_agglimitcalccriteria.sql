...
2026-01-01      0.0                             Incremental Curated model for cctl_agglimitcalccriteria.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_agglimitcalccriteria')) }}
    FROM {{ ref('stg_cctl_agglimitcalccriteria') }}
    {% if is_incremental() %}
