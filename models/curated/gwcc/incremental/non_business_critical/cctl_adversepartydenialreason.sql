...
2026-01-01      0.0                             Incremental Curated model for cctl_adversepartydenialreason.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_adversepartydenialreason')) }}
    FROM {{ ref('stg_cctl_adversepartydenialreason') }}
    {% if is_incremental() %}
