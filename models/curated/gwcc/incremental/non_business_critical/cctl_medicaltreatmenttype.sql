...
2026-01-01      0.0                             Incremental Curated model for cctl_medicaltreatmenttype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_medicaltreatmenttype')) }}
    FROM {{ ref('stg_cctl_medicaltreatmenttype') }}
    {% if is_incremental() %}
