...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequeststatement.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequeststatement')) }}
    FROM {{ ref('stg_cctl_servicerequeststatement') }}
    {% if is_incremental() %}
