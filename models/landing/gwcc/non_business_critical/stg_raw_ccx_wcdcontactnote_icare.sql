...
2026-01-01      0.0                             Incremental staging model for ccx_wcdcontactnote_icare.
                                                wcdcontactnote_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_wcdcontactnote_icare"]
) }}
...
(
            ...
            SELECT
                TO_TIMESTAMP_TZ(data_payload:NoteDate::NUMBER/1000) AS notedate,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_wcdcontactnote_icare') }}
            WHERE REGEXP_SUBSTR(metadata_file_name, '[^.]+$') = 'avro'
            UNION ALL 
            SELECT
                $1:notedate::TIMESTAMP_TZ AS notedate,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS wcdcontactnote_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'notedate',
                        'loadcommandid',
