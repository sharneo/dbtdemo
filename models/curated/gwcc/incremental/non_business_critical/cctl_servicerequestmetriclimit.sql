...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequestmetriclimit.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequestmetriclimit')) }}
    FROM {{ ref('stg_cctl_servicerequestmetriclimit') }}
    {% if is_incremental() %}
