...
2026-01-01      0.0                             Incremental staging model for ccx_toocsnoiroivalid_icare.
                                                toocsnoiroivalid_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_toocsnoiroivalid_icare"]
) }}
...
(
            ...
            SELECT
                ...
                CAST(data_payload:ResultOfInjuryCode::TEXT AS VARCHAR(255)) AS resultofinjurycode,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_toocsnoiroivalid_icare') }}
            ...
            SELECT
                ...
                CAST($1:resultofinjurycode::TEXT AS VARCHAR(255)) AS resultofinjurycode,
                $1:id::NUMBER AS id,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS toocsnoiroivalid_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'natureofinjurycode',
                        'createuserid',
