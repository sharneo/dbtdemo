...
2026-01-01      0.0                             Incremental Curated model for ccx_infosettconf_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_infosettconf_icare')) }}
    FROM {{ ref('stg_ccx_infosettconf_icare') }}
    {% if is_incremental() %}
