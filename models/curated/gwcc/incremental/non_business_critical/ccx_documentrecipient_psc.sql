...
2026-01-01      0.0                             Incremental Curated model for ccx_documentrecipient_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_documentrecipient_psc')) }}
    FROM {{ ref('stg_ccx_documentrecipient_psc') }}
    {% if is_incremental() %}
