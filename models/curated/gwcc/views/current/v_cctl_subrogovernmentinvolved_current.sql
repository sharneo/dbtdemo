...
2026-01-01      0.5                             Curated current-state view for cctl_subrogovernmentinvolved.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_subrogovernmentinvolved_current')) }}
    FROM {{ ref('dt_cctl_subrogovernmentinvolved_current') }}
