...
2026-01-01      0.0                             Incremental Curated model for cctl_mattermethodserved.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_mattermethodserved')) }}
    FROM {{ ref('stg_cctl_mattermethodserved') }}
    {% if is_incremental() %}
