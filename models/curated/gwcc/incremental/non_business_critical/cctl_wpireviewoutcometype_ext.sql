...
2026-01-01      0.0                             Incremental Curated model for cctl_wpireviewoutcometype_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_wpireviewoutcometype_ext')) }}
    FROM {{ ref('stg_cctl_wpireviewoutcometype_ext') }}
    {% if is_incremental() %}
