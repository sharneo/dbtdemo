...
2026-01-01      0.0                             Incremental staging model for ccx_wcdwiroreview_icare.
                                                wcdwiroreview_icare_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_wcdwiroreview_icare"]
) }}
...
(
            ...
            SELECT
                ...
                TO_TIMESTAMP_TZ(data_payload:DateReviewCompleted::NUMBER/1000) AS datereviewcompleted,
                CAST(data_payload:Reviewer::TEXT AS VARCHAR(255)) AS reviewer,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_wcdwiroreview_icare') }}
            ...
            SELECT
                ...
                $1:datereviewcompleted::TIMESTAMP_TZ AS datereviewcompleted,
                CAST($1:reviewer::TEXT AS VARCHAR(255)) AS reviewer,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS wcdwiroreview_icare_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
