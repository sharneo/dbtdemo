...
2026-01-01      0.0                             Incremental Curated model for cctl_effdatedchangetype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_effdatedchangetype')) }}
    FROM {{ ref('stg_cctl_effdatedchangetype') }}
    {% if is_incremental() %}
