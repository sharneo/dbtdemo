...
2026-01-01      0.0                             Incremental Curated model for ccx_managentityonorganisation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_managentityonorganisation')) }}
    FROM {{ ref('stg_ccx_managentityonorganisation') }}
    {% if is_incremental() %}
