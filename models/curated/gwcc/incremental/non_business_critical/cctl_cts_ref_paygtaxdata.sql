...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_ref_paygtaxdata.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_ref_paygtaxdata')) }}
    FROM {{ ref('stg_cctl_cts_ref_paygtaxdata') }}
    {% if is_incremental() %}
