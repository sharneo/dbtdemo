...
2026-01-01      0.0                             Incremental Curated model for cctl_cts_schedule3item.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_cts_schedule3item')) }}
    FROM {{ ref('stg_cctl_cts_schedule3item') }}
    {% if is_incremental() %}
