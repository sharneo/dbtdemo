...
2026-01-01      0.0                             Incremental Curated model for cctl_dashboardstattype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_dashboardstattype')) }}
    FROM {{ ref('stg_cctl_dashboardstattype') }}
    {% if is_incremental() %}
