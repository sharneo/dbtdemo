...
2026-01-01      0.0                             Incremental Curated model for ccx_camt053mappinghistory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_camt053mappinghistory')) }}
    FROM {{ ref('stg_ccx_camt053mappinghistory') }}
    {% if is_incremental() %}
