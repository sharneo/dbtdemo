...
2026-01-01      0.5                             Dynamic Table for current-state of bctl_bulktroubletktdataloadtype_ext.
...
{{ config(
    ...
    tags=["curated", "gwbc", "billing_centre", "non_business_critical", "dynamic_table"]
) }}

SELECT
    {{ dbt_utils.star(from=ref('v_bctl_bulktroubletktdataloadtype_ext')) }}
FROM {{ ref('v_bctl_bulktroubletktdataloadtype_ext') }}
