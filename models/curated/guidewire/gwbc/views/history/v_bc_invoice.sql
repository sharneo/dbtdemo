...
2026-01-01      0.0                             Curated SCD2 full history view for bc_invoice.
                                                Source: {{ ref('bc_invoice_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "billing_centre", "non_business_critical", "bc_invoice"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('bc_invoice_snapshot')) }}
    FROM {{ ref('bc_invoice_snapshot') }}
),
