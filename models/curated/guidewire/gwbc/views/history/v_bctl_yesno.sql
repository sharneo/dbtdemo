...
2026-01-01      0.0                             Curated SCD2 full history view for bctl_yesno.
                                                Source: {{ ref('bctl_yesno_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "billing_centre", "non_business_critical", "bctl_yesno"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('bctl_yesno_snapshot')) }}
    FROM {{ ref('bctl_yesno_snapshot') }}
),
