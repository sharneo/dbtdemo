...
2026-01-01      0.5                             Curated current-state view for ccx_courtawardpayment_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_ccx_courtawardpayment_ext_current')) }}
    FROM {{ ref('dt_ccx_courtawardpayment_ext_current') }}
