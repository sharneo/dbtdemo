...
2026-01-01      0.5                             Curated current-state view for ccx_documentrecipient_psc.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_ccx_documentrecipient_psc_current')) }}
    FROM {{ ref('dt_ccx_documentrecipient_psc_current') }}
