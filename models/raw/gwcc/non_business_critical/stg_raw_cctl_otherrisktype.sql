...
2026-01-01      0.0                             Incremental staging model for cctl_otherrisktype.
                                                otherrisktype_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "cctl_otherrisktype"]
) }}
...
(
            ...
            SELECT
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'cctl_otherrisktype') }}
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS otherrisktype_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'l_en_us',
                        'priority',
                        's_en_us_edg_ph',
