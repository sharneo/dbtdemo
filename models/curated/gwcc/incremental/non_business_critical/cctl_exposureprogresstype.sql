...
2026-01-01      0.0                             Incremental Curated model for cctl_exposureprogresstype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_exposureprogresstype')) }}
    FROM {{ ref('stg_cctl_exposureprogresstype') }}
    {% if is_incremental() %}
