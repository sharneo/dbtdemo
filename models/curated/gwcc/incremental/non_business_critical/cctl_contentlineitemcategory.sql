...
2026-01-01      0.0                             Incremental Curated model for cctl_contentlineitemcategory.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contentlineitemcategory')) }}
    FROM {{ ref('stg_cctl_contentlineitemcategory') }}
    {% if is_incremental() %}
