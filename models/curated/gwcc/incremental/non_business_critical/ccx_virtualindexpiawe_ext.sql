...
2026-01-01      0.0                             Incremental Curated model for ccx_virtualindexpiawe_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_ccx_virtualindexpiawe_ext')) }}
    FROM {{ ref('stg_ccx_virtualindexpiawe_ext') }}
    {% if is_incremental() %}
