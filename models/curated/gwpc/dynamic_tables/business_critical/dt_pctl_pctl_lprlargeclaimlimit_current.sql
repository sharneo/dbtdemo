...
2026-01-01      0.5                             Dynamic Table for current-state of pctl_pctl_lprlargeclaimlimit.
...
{{ config(
    ...
    tags=["curated", "gwpc", "policy_centre", "business_critical", "dynamic_table"]
) }}

SELECT
    {{ dbt_utils.star(from=ref('v_pctl_pctl_lprlargeclaimlimit')) }}
FROM {{ ref('v_pctl_pctl_lprlargeclaimlimit') }}
