...
2026-01-01      0.0                             Incremental Curated model for cctl_freetextclaimsearchtype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_freetextclaimsearchtype')) }}
    FROM {{ ref('stg_cctl_freetextclaimsearchtype') }}
    {% if is_incremental() %}
