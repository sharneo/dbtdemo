...
2026-01-01      0.0                             Incremental Curated model for cctl_benefitendreasontype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_benefitendreasontype')) }}
    FROM {{ ref('stg_cctl_benefitendreasontype') }}
    {% if is_incremental() %}
