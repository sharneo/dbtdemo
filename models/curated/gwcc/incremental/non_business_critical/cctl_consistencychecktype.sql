...
2026-01-01      0.0                             Incremental Curated model for cctl_consistencychecktype.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_consistencychecktype')) }}
    FROM {{ ref('stg_cctl_consistencychecktype') }}
    {% if is_incremental() %}
