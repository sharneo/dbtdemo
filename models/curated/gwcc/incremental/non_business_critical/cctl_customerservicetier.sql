...
2026-01-01      0.0                             Incremental Curated model for cctl_customerservicetier.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_customerservicetier')) }}
    FROM {{ ref('stg_cctl_customerservicetier') }}
    {% if is_incremental() %}
