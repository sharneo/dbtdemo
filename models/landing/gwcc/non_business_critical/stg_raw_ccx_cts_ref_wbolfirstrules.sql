...
2026-01-01      0.0                             Incremental staging model for ccx_cts_ref_wbolfirstrules.
                                                cts_ref_wbolfirstrules_sk: Entity identity surrogate key on PK ('id')
...
{{ config(
    ...
    tags=["raw_layer", "raw_claim_centre", "claim_centre", "non_business_critical", "ccx_cts_ref_wbolfirstrules"]
) }}
...
(
            ...
            SELECT
                ...
                CAST(data_payload:DaysWorked::TEXT AS VARCHAR(50)) AS daysworked,
                ...
                'AVRO' file_type
            FROM {{ source('gwcc', 'ccx_cts_ref_wbolfirstrules') }}
            ...
            SELECT
                ...
                CAST($1:daysworked::TEXT AS VARCHAR(50)) AS daysworked,
    ...
    SELECT
        *,
        CAST({{ dbt_utils.generate_surrogate_key([
            'id'
        ]) }} AS VARCHAR(150)) AS cts_ref_wbolfirstrules_sk,
        CAST({{ dbt_utils.generate_surrogate_key([
                        'loadcommandid',
                        'createuserid',
                        'publicid',
