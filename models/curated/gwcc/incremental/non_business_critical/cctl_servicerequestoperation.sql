...
2026-01-01      0.0                             Incremental Curated model for cctl_servicerequestoperation.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_servicerequestoperation')) }}
    FROM {{ ref('stg_cctl_servicerequestoperation') }}
    {% if is_incremental() %}
