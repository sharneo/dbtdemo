...
2026-01-01      0.0                             Incremental Curated model for cctl_contactdestructstatuscat.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_contactdestructstatuscat')) }}
    FROM {{ ref('stg_cctl_contactdestructstatuscat') }}
    {% if is_incremental() %}
