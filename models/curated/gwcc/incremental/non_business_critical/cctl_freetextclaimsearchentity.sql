...
2026-01-01      0.0                             Incremental Curated model for cctl_freetextclaimsearchentity.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_freetextclaimsearchentity')) }}
    FROM {{ ref('stg_cctl_freetextclaimsearchentity') }}
    {% if is_incremental() %}
