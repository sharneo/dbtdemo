...
2026-01-01      0.5                             Curated current-state view for abtl_contactstatus_ext.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_abtl_contactstatus_ext_current')) }}
    FROM {{ ref('dt_abtl_contactstatus_ext_current') }}
