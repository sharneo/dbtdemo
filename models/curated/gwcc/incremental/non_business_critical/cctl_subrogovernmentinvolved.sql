...
2026-01-01      0.0                             Incremental Curated model for cctl_subrogovernmentinvolved.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_subrogovernmentinvolved')) }}
    FROM {{ ref('stg_cctl_subrogovernmentinvolved') }}
    {% if is_incremental() %}
