...
2026-01-01      0.0                             Curated SCD2 full history view for cc_role.
                                                Source: {{ ref('cc_role_snapshot') }}
...
{{ config(
    materialized='view',
    tags=["curated", "curated_view", "curated_history", "claim_centre", "non_business_critical", "cc_role"]
) }}

WITH cte_source_data AS (
    SELECT
        {{ dbt_utils.star(from=ref('cc_role_snapshot')) }}
    FROM {{ ref('cc_role_snapshot') }}
),
