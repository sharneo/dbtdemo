...
2026-01-01      0.5                             Curated current-state view for pcx_losshistforrating_icare.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_pcx_losshistforrating_icare_current')) }}
    FROM {{ ref('dt_pcx_losshistforrating_icare_current') }}
