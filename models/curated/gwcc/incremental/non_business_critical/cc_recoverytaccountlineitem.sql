...
2026-01-01      0.0                             Incremental Curated model for cc_recoverytaccountlineitem.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cc_recoverytaccountlineitem')) }}
    FROM {{ ref('stg_cc_recoverytaccountlineitem') }}
    {% if is_incremental() %}
