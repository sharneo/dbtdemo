...
2026-01-01      0.0                             Incremental Curated model for cctl_partialdenialreason.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('stg_cctl_partialdenialreason')) }}
    FROM {{ ref('stg_cctl_partialdenialreason') }}
    {% if is_incremental() %}
