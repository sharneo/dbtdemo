...
2026-01-01      0.5                             Curated current-state view for cctl_claimagent_icare_replica.
...
(
    SELECT
        {{ dbt_utils.star(from=ref('dt_cctl_claimagent_icare_replica_current')) }}
    FROM {{ ref('dt_cctl_claimagent_icare_replica_current') }}
